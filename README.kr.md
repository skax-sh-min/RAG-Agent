...
| `AUTH_ENABLED` | — | `true` | Spring Security 폼 로그인 on/off(`app.auth.enabled`). `false`면 no-login 배포 — CSRF 끄고 stateless 세션, `NoAuthAutoLoginFilter`가 게스트/관리자 신원을 주입하며 관리자가 생기기 전까지 `/setup`으로 보냅니다 — [OPERATOR_MANUAL.md §9.4](documents/OPERATOR_MANUAL.md) 참고 |
| `AUTH_MANAGEMENT_ONLY` | — | `false` | `AUTH_ENABLED=false`일 때만 의미 있음(`app.auth.management-only`). 채팅·조회는 게스트에 열어두고 `/admin/**`과 문서 쓰기 라우트만 실제 로그인을 요구합니다(게이트는 `hasRole("ADMIN")`이지 단순 인증이 아닙니다) |
...
| `LLM_MAX_TOKENS` | — | `10000` | **블로킹** LLM 호출(분류·키워드 추출·MD 교정·TXT 구조화 등) 전용 completion 길이 상한 — 스트리밍 채팅/Direct 답변은 이 값과 무관(대신 SSE 타임아웃이 제한). 대화 히스토리 예산·MD 교정 섹션 분할 크기도 같은 값을 공유. **검증 호출만은 예외** — 자체 상한 2,048토큰을 쓴다(JSON 몇 필드짜리 응답이 좁은 컨텍스트에서 completion 예산 전체를 예약하면 안 되므로). **모델 컨텍스트 윈도우 자체가 아님** — 실제 LLM 서버 컨텍스트(`--ctx-size`)에 여유를 두고 설정할 것. 기본값 `10000`은 **32k 이상** 컨텍스트를 전제하므로, 8k~10k 모델이면 ~2000으로 낮추세요 — [PIPELINE.md §4.1](documents/PIPELINE.md#41-appllmmax-tokensllm_max_tokens-크기-산정--로컬-llm-컨텍스트-윈도우와의-관계) · [OPERATOR_MANUAL §8](documents/OPERATOR_MANUAL.md#8-문제-해결) 참고 `/settings`에서 핫 수정 가능(1,000~32,000). |
...
| `LLM_INDEXING_TEMPERATURE` | — | `0.0` | 모든 ungated 백그라운드/인덱싱 호출 전용 temperature(`app.llm.indexing-temperature`) — 키워드 추출, MD 교정, TXT 구조화, 이미지 설명/분류, 스레드 제목 생성, 대화 요약. `LLM_TEMPERATURE`/`DIRECT_LLM_TEMPERATURE`와 별개라 그 값이 무엇이든 이 추출/분류형 호출은 결정적으로 유지됨. `[0.0, 0.1]`으로 clamp — 출력이 파싱되는 추출 작업이라 표본추출의 다양성이 이득이 아니라 결함이고, 0에서 반복에 빠지는 로컬 모델을 깨울 만큼만 열어 둠. **`/settings`에서 핫 수정** — 재기동 없이 다음 호출부터 적용 |
| `CREATIVE_LLM_TEMPERATURE` | — | `0.7` | **C(응용) 응답 모드 전용** temperature(`app.llm.creative-temperature`), `[0.0, 1.0]` clamp. `LLM_TEMPERATURE`가 `[0.0, 0.3]`으로 묶여 있어(문서 충실 답변이 표본추출로 흔들리면 안 된다) 창의 생성이 원천 봉쇄되기 때문에 따로 둔 값. **`/settings`에서 핫 수정 가능**하며 블로킹·스트리밍 **양쪽**에 적용됨 |
| `LLM_SHRINK_STEP` | — | `1` | 컨텍스트 초과로 프롬프트가 거절되면 재시도마다 덜어낼 문서 수(`app.llm.shrink-step`). 사전 예산이 이미 창에 맞춰 놓은 뒤라 여기까지 오는 초과는 대개 아슬아슬하므로 반씩이 아니라 한두 개씩 줄인다. 재시도 상한이 5회라 **이 값 × 5** 가 도달 가능한 최대 축소폭이고, 거기서도 안 되면 `RAG-LLM-003`. 핫 수정 가능 |
| `CREATIVE_MODE_ENABLED` | — | `true` | **C(응용) 응답 모드를 열지 말지**(`app.llm.creative-mode-enabled`). 위 온도가 "C가 **어떻게** 답할까"라면 이 값은 "C를 **열어 둘까**" — 검색된 문서 밖의 내용을 생성하는 유일한 모드라 배포처에 따라 처음부터 제공하지 않는 편이 맞을 수 있다. `false`면 채팅 입력창에서 C 버튼이 (비활성이 아니라) **아예 사라지고**, 그럼에도 도착한 요청(REST·손으로 만든 폼)은 서버가 `N`으로 강등한다. 이미 C로 답한 과거 턴의 `[C]` 표기·배지는 그대로 남는다. **`/settings`에서 핫 수정 가능** |
...
| `UPLOAD_MAX_TOTAL_SIZE` | — | `0`(무제한) | 배포 전체 저장 상한. `20GB`/`500MB`처럼 단위를 붙일 수 있고 접미사가 없으면 바이트. `DATA_DIR` 아래 `documents`+`converted`+`images`를 센다 — 업로드 원본만이 아니라 인덱싱이 만드는 변환 MD·추출 이미지까지. 초과 시 업로드는 413 `RAG-UP-002`로 거부된다. `documents/backup/`은 제외라(아래) 문서를 지우면 실제로 자리가 난다 — [OPERATOR_MANUAL.md](documents/OPERATOR_MANUAL.md#업로드-크기-제한) 참고 |
| `BACKUP_RETENTION_DAYS` | — | `30` | 문서 삭제 시 원본이 보관되는 `documents/backup/`의 보관 일수. `0` = 기간 제한 없음 |
| `BACKUP_MAX_SIZE` | — | `2GB` | 같은 폴더의 용량 상한 — 초과 시 오래된 것부터 삭제. `0` = 용량 제한 없음. 이 앱이 만든 백업만 지운다 |
...
| `SEARCH_TOP_K` | `10` | 2 ~ 15 | 벡터 검색 반환 문서 수. 검증 호출에 실리는 발췌량(`topK × CHUNK_SIZE`, 이 앱 최대의 단일 요청)도 이 값이 정하므로, 모델 컨텍스트가 좁으면 여기부터 낮춘다 |
| `SEARCH_SIMILARITY_THRESHOLD` | `0.3` | 0.0 ~ 0.75 | 청크 유지 최소 코사인 유사도 (`0.0`=전체 수용). **벡터 축에만** 걸린다(`VectorStoreProvider` 내부라 큐레이션 축도 함께 걸러지지만 BM25는 아님) — 올리면 키워드 축의 상대 비중이 함께 올라가므로 `SEARCH_RRF_KEYWORD_WEIGHT`와 같이 올리지 말 것 |
| `SEARCH_MULTIQUERY_ENABLED` | `true` | true/false | 검색 전 질의 전처리 on/off — 멀티쿼리 확장과 아래 §10.12 후속 질문 독립화를 **함께** 켜고 끕니다(같은 성질의 호출이라 스위치도 하나) |
| `SEARCH_MULTIQUERY_MIN_LENGTH` | `15` | 0 ~ 20 | 단순한 생략 임계값이 아니라 **질의 전처리 두 종류의 경계**입니다. **이상**이면 멀티쿼리 확장(이때 원본 질의 검색은 확장을 기다리지 않고 병렬 실행), **미만**이면 §10.12 독립화 — `"그 설정은 어디야?"` 같은 짧은 후속 질문을 최근 **질문들**(답변은 쓰지 않습니다)로 자립적인 검색어로 다시 쓰고, 그 한 값을 벡터·BM25·큐레이션 축과 리랭커·분류기가 함께 씁니다(답변 프롬프트의 질문은 원문 그대로). 둘은 여집합이라 한 턴에 이런 호출을 둘 쓰지 않습니다. `0`=확장만, 독립화는 영영 안 함 |
| `SEARCH_HYBRID_ENABLED` | `true` | true/false | RRF에 BM25(FTS5) 키워드 축 추가 (§10.7.2 — FTS 인덱스는 이 플래그와 무관하게 인덱싱 시점에 채워지므로 재인덱싱 불필요) |
| `SEARCH_RETRY_ESCALATE` | `true` | true/false | **재검색** 에스컬레이션 — 플래그 하나로 두 축: 후보 풀 `round(topK × (1 + 0.5 × 재검색횟수))`(상한 `×3`) **및** 최종 컷 `topK + 재검색횟수`(검증 호출에 여유가 있을 때만 적용). 후보 풀만 키우면 같은 topK 자리를 놓고 경쟁하는 문서만 바뀌어, 컷 바로 뒤의 근거는 매 시도 계속 밖에 머문다. 재시도는 직전 컷에서 **근거로 쓰이지 않은 하위 청크를 최대 1/3 밀어내기도** 하므로, 후보 풀을 예전처럼 2배로 키울 필요가 없어졌다 |
...
**RAG 턴**의 대화 이력 주입 길이는 `LLM_MAX_TOKENS × 0.5`(최소 1,000자)로 자동 계산됩니다. 원문 그대로 보내는 폴백 경로와 아래 요약 캐시 경로 모두 같은 예산을 받으므로, 두 경로 사이를 오가도 LLM에 전달되는 컨텍스트 *양*은 달라지지 않습니다.

**Direct 턴은 예산이 다릅니다**(§10.13). 프로바이더의 컨텍스트 창에서 `입력 예산 − 문서가 차지할 자리`로 파생되며, 검색이 돌지 않으므로 그 자리가 0입니다 — 즉 RAG 값보다 클 수도, **작을 수도** 있습니다(창이 좁으면 정직한 값이 기존 고정 상한보다 작고, 원래 Direct 경로에는 예산 가드가 아예 없었습니다). 예산 때문에 이력이 잘리면 답변이 그 사실을 말합니다(`budgetNote`). 두 경로가 읽는 규칙은 `HistoryPolicy` 하나이고, 각 이전 턴을 요약으로 실을지 본문으로 실을지도 거기서 정합니다 — [PIPELINE.md §3.1](documents/PIPELINE.md) 참고.
...
└── src/main/
    ...
    │   │   └── WebConfig.java         # ChatClient 빈 + i18n (CookieLocaleResolver); CORS 매핑은 일부러 두지 않는다
    │   ├── audit/
    ...
    │   │   ├── AdminController.java            # /admin, /admin/chunks; 문서 재인덱스; 큐레이션 Q&A + 지식 제안 검토
    │   │   ├── CuratedSubmissionController.java # /curated/submissions — 지식 제안 게시판(등록·철회·본문 이미지 업로드·미확인 배지)
    ...
    │   │   ├── CuratedSubmissionRepository.java   # curated_submission — 지식 제안 게시판(검토 대기/등록/반려)
    ...
    │       ├── CuratedSubmissionService.java  # 지식 제안 게시판: 입력 검증+태그, 승인 시 분할(1:N), 거부, 알림 카운트
    ...
    └── resources/
        ...
        └── templates/
            ...
            └── fragments/
                ...
                ├── admin-submissions.html # 관리자 지식 제안 검토 패널 (지연 로딩, 상태 필터)
  ...
  └─▶ [Classifier]  → 질문 유형 분류 (concept / usage / error / version / meta)
        ...
        └─ other ──▶ [Retrieval]   (LLM 최적 쿼리 생성 → 벡터 검색)
                       └─▶ [Answer]   (구조화 답변 + sufficient 자기평가)
                              ├─ 증거 부족 ──▶ [Retrieval] (재검색, 최대 2회)
                              └─ 충분    ──▶ [Finalize] (response mode = S)
                                           └─▶ [Critic]   (response mode != S, 근거 검증)
                                             ├─ 미근거  ──▶ [Answer]  (재검색 없이 같은 근거로 답변만 재작성)
                                             └─ 근거 OK ──▶ [Finalize] → 응답
```

## 주요 기능

- **인증** — Spring Security 폼 로그인, BCrypt(12) 비밀번호 해싱, 5회 실패 시 15분 계정 잠금, `/login`·`/signup`·`/setup`. `app.auth.enabled=false`로 로컬 no-login 배포 가능; `app.auth.management-only=true`는 채팅·조회는 게스트에 열어두고 문서 관리·`/admin`만 로그인 요구 — [OPERATOR_MANUAL.md §9.4.2](documents/OPERATOR_MANUAL.md#942-관리-전용-인증-management-only) 참고. `/setup`(최초 관리자 생성)은 **모든 모드에서 열린다** — `ROLE_ADMIN` 계정을 만드는 유일한 경로이고(`/signup`은 `ROLE_USER`만 만든다) 관리자가 없으면 아래 게이트를 아무도 통과할 수 없기 때문. 관리자가 생기면 스스로 닫힌다
...
- **컨텍스트 창 인지** — 프로바이더마다 창 크기를 선언(`app.llm.providers[N].context-size`)하거나 기동 시 탐지한다(llama.cpp `/props`, LM Studio `/api/v0/models`). 두 서버가 함께 노출하는 "모델 상한" 필드는 **일부러 읽지 않는다** — `-c 8192`로 띄운 서버도 131072를 돌려주므로, 그 값으로 예산을 짜면 컨텍스트 초과를 막으려던 코드가 오히려 초과를 부른다. 창을 알면 호출 **전에** 프롬프트를 맞춘다(최저 관련도 문서부터, 그다음 가장 오래된 대화 턴. 최상위 문서는 항상 남긴다). 무엇이 빠졌는지는 답변 아래 한 줄과 출처별 `미사용` 배지로 알린다 — 출처 목록은 검색된 전부를 보여주므로 이 표시가 없으면 모델이 읽지 않은 문서를 읽은 것처럼 보게 된다. 창을 모르면 **아무것도 자르지 않는다**(추측한 숫자로 근거를 버리는 것이 초과보다 나쁘다). 컨텍스트 초과는 더 이상 서킷 브레이커를 건드리지 않는다 — 결정적 오류라 유일한 로컬 프로바이더를 막으면 통과했을 작은 요청까지 함께 죽는다
- **응답 모드 (S / N / C)** — 메시지마다 고르는 **답변의 성격** 축(라우팅 모드와 직교). **S**(간단히)는 `## 요약` 한 섹션 1,000자 이내(문서 발췌가 없어 답변이 스스로 풀어 써야 하는 Direct 경로에서는 1,500자)로 검증 자체를 건너뛰고, **N**(자세히, 기본값)은 문서에 충실한 5섹션 답변, **C**(응용)는 검색된 문서를 **재료로** 코드·설정을 만들어내되 문서에서 온 것과 모델이 채운 것을 섹션으로 분리한 4섹션 형식이다. **모드마다 시스템 프롬프트를 통째로 갖는다** — 공용 프롬프트에 사용자 메시지로 지시를 덮어쓰는 방식은 S에서 실패했다(5섹션 헤더 목록이 한 줄 지시보다 강해 모델이 전부 생성했고, 서버가 사후에 잘라 화면과 저장본이 갈렸다). **숫자를 말하는 건 S뿐이다**: 짧은 출력에 건 상한은 모델이 스스로 멈추는 지점보다 앞에 있어 구속력이 있지만, 긴 출력에 건 목표는 그 뒤에 있어 아무 일도 하지 않는다(제거된 M/L이 약 5,000자·10,000자를 요청하고 실제 3,047자·3,187자를 냈다 — 그 실측이 L을 없앤 근거다). N·C는 숫자 대신 무엇을 다룰지를 지시한다. `ResponseMode.maxTokens()`가 내는 모드별 예산(S 15%/2,000자 하한, N·C 70%/5,000자, `LLM_MAX_TOKENS`를 절대 초과하지 않음)은 **블로킹 호출에만** 걸리는 폭주 방지 안전판이고 스트리밍에는 토큰 상한이 없다 — `/settings`가 모드별 실효 예산과 어느 항이 이겼는지를 보여준다. **C는 Direct와 배타**(검색 결과가 전제)이며 클라이언트와 서버가 각각 독립적으로 막는다. C는 큐레이션과 답변 재사용에서도 제외된다(아래 참고). 또한 **C는 운영자가 통째로 끌 수 있는 유일한 모드**다(`CREATIVE_MODE_ENABLED`, `/settings`에서 핫 수정) — 문서에 없는 것을 쓰는 유일한 모드라 제공 여부 자체가 운영 정책이기 때문이다. 끄면 버튼이 사라지고 서버가 요청을 `N`으로 강등하되, 이미 C로 답한 기록은 그대로 남는다
...
- **큐레이션 Q&A (§10.10 · §10.11)** — 공유 지식 축. 별도로 임베딩되어(예약 벡터스토어 버전 네임스페이스, 문서 재인덱싱에도 보존) 이후 검색에 가중 RRF 축으로 융합됨(`SEARCH_CURATED_QA_ENABLED`/`SEARCH_CURATED_QA_WEIGHT`, `/settings`에서 핫 수정 가능). 정답을 그대로 반환하지 않고 근거로만 주입해 LLM이 현재 문서와 대조함. **여기로 들어오는 문은 하나다 — 관리자의 지식 제안 승인.** 답변에 좋아요를 누르면 그 답변으로 채워진 제안 폼이 열릴 뿐, 좋아요 자체는 아무것도 등록하지 않는다(예전에는 곧바로 등록되어 3초 뒤 검색에 반영됐다 — 문서를 하나도 안 본 답변이 클릭 한 번에 전체 검색 지식이 될 수 있었다). 저자는 `/curated/submissions` 에서 자기 제안을 수정·철회하고, 관리자는 `/admin` 에서 전체 항목을 검토·편집·강제 삭제한다. **S 모드 답변은 좋아요를 눌러도 제안을 만들 수 없다** — 배경·이유를 일부러 덜어낸 형식이라 오래 남길 지식의 원본이 아니기 때문. 싫어요는 그대로 동작한다. 승격된 답변은 **질문 당시의 검색 스코프(태그)를 승계**해(turn 단위로 저장) 태그를 좁힌 검색에서도 살아남고, 스코프를 알 수 없는 항목은 어느 스코프에도 속하지 않는 대신 **모든 스코프에 속한 것으로** 취급된다 — 이 처리가 없던 때는 태그 칩을 하나라도 켜면 큐레이션 항목이 전부 결과에서 사라졌다
- **지식 제안 게시판 (사용자 제안 → 관리자 임베딩)** — 사용자가 `/curated/submissions`에서 제목+본문으로 지식을 등록하면, 관리자가 `/admin` 전용 카드에서 검토해 **임베딩 실행**하거나 **사유를 적어 거부**한다. 승인된 제안은 좋아요 큐레이션 Q&A와 같은 검색 축으로 들어가므로 `SEARCH_CURATED_QA_*` 설정이 그대로 적용되고, 게시판 자체는 별도 테이블이라 `curated_qa.status='active'`가 "지금 검색에 기여 중"이라는 의미를 그대로 유지한다. **관리자 승인이 사용자 작성 텍스트와 답변 프롬프트의 `[검색된 문서]` 블록 사이의 유일한 관문**이라, 검토 화면은 항상 본문 전문을 보여주고 일괄·자동 승인 경로는 의도적으로 만들지 않았다. 알림은 양방향 60초 헤더 배지 폴링 — 관리자에게는 검토 대기 건수(로그인 직후 첫 폴링에 바로 표시), 작성자에게는 처리 결과("내 제안"을 열면 읽음 처리). **본문 길이 제한은 없다**: 승인 시 본문이 `ChunkSplitter`를 그대로 통과해 N개 큐레이션 행으로 등록되므로(문서 인덱싱과 같은 기계라 `CHUNK_SPLIT_GRANULAR`·표/코드 블록 보호까지 적용) "너무 길어 임베딩 불가"라는 실패 모드가 입력을 거부하는 대신 구조적으로 사라진다. 상태는 **전부/전무** — 청크가 하나라도 살아 있으면 등록 완료, 전부 내려가면 회수됨이며 청크 하나를 지우면 제안 전체가 함께 내려간다(반쪽 등록 상태가 생기지 않음). 본문은 마크다운으로 작성·미리보기하고(등록 폼·관리자 검토 화면 모두 DOMPurify 경유), 태그를 달면 그 태그를 고른 검색에만 반영되며 태그가 없으면 모든 스코프에서 검색된다. [OPERATOR_MANUAL.md §6.9](documents/OPERATOR_MANUAL.md#69-청크-추가-게시판-사용자-제안--관리자-임베딩) 참고
...
- **ReAct 재검색** — 검증 실패 시 최대 2회 자동 재시도하되, 두 실패를 다르게 다룬다: *근거 부족*(질문에 답하지 못함)은 다시 검색하고, *근거 이탈*(문서 밖으로 나감)은 이미 손에 있는 근거로 답변만 다시 쓴다. 재시도 프롬프트에는 평가가 낸 반려 사유 한 문장이 실린다 — 없으면 온도 0에서 같은 입력을 결정적으로 반복하는 것과 같다. 재검색은 후보 풀과 최종 컷을 넓히는 동시에 **근거로 쓰이지 않은 하위 청크를 밀어내어**, 새 근거가 죽은 무게와 경쟁하는 대신 자리를 받게 한다
- **Critic 검증** — 충분도와 근거를 한 번의 평가 호출로 함께 판정하고 CRITIC 노드는 그 결과를 읽기만 한다(별도 LLM 왕복 없음). 판정 대상은 **답변이 실제로 쓴 것과 같은 문서 전체** — 예전에는 답변이 `SEARCH_TOP_K` 전부를 쓰는데 검증은 앞 5개만 봐서, 6~8번째 문서에만 있는 값을 정확히 인용한 답변이 근거 없음으로 판정됐다. 두 프롬프트가 같은 정규화 형태의 값을 보는 것도 함께 맞췄다. response mode가 S인 turn은 CRITIC 단계를 생략하므로 재시도는 ANSWER의 sufficient 판단 기준으로만 결정됨. C는 검증을 **끄지 않고 바꿔 낀다** — "답변이 문서에 근거하는가"는 창의 답변에서 정의상 항상 거짓이라 그대로 두면 재시도 예산을 전부 태우므로, 대신 "문서 유래라고 제시한 이름이 실재하는가"를 묻고 발명된 이름은 재시도가 아니라 경고로 보고한다. **검증 결과를 읽지 못하면 '통과'가 아니라 '판정 없음'이다** — 평가 호출이 빈 응답을 주거나 JSON 파싱이 실패하면 배지를 아예 붙이지 않고(`검증됨`/`생성`도, `미검증`도 아님) 재시도도 걸지 않는다. 예전 폴백은 그 자리에 `grounded=true`를 써넣어 **검증한 적 없는 답변에 초록 검증됨 배지**를 달았다
...
- **PROGRESSIVE 모드** — COST_FIRST로 시작 → 검증이 `sufficient=false`를 내고 **재시도까지 소진했을 때** PREMIUM 프로바이더로 재실행 + 업그레이드 배지 표시. 품질 점수도 임계값 프로퍼티도 없습니다 — `app.llm.progressive-threshold`는 읽는 코드가 없어 2026-09-01에 제거됐습니다
...
- **클라이언트 IP 신뢰 판정 일원화** — `app.trust-forwarded-for`(기본 `false`)가 속도 제한·방문자 식별·**감사 로그** 세 곳에서 `X-Forwarded-For` 신뢰 여부를 결정. 끄면 헤더를 위조해도 속도 제한을 리필하거나 다른 방문자 신원을 가로채거나 "누가 했는가"의 기록에 거짓 IP를 남길 수 없고, 켜면(Caddy 등 프록시 뒤에서는 필수) 모든 방문자가 프록시 IP 하나로 뭉치지 않고 실제 IP로 식별됨
- **로그 입력값 상한** — 클라이언트가 보낸 `X-Trace-Id`는 id 모양일 때만(영숫자와 `_-`, 최대 64자) 받아들이고 아니면 새로 발급. 이 값은 모든 로그 줄에 찍히므로 검증이 없으면 줄바꿈 하나로 가짜 로그 줄을 만들거나 긴 값이 매 줄에 반복돼 로그 파일이 부푼다
- **비-root 컨테이너** — 이미지가 UID 10001로 실행된다. 기존 Linux 배포를 올릴 때는 `chown -R 10001:10001 ./data`가 한 번 필요하며, 엔트리포인트가 쓰기 권한을 먼저 확인해 SQLite 오류 대신 그 명령을 그대로 안내한다
