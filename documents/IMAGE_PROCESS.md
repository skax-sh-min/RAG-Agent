...
- 그룹/커넥터/텍스트없는 도형 등 "그리기 도구" 요소(아래 `PPTX_IMAGE_RASTERIZE_SHAPES` 참고)
...
- **사진 위 겹친 주석 도형** (`app.pptx-image.merge-annotated-pictures`, 기본 `true`, `PPTX_IMAGE_RASTERIZE_SHAPES`와 **독립**): 화면 캡처 위에 그린 강조 원·화살표 같은 시드 도형을 사진과 하나의 합성 PNG로 만들어, "깨끗한 사진 + 맥락 없이 떠 있는 주석"으로 따로 추출되던 문제를 없앱니다. 겹친 시드가 없는 사진은 원본 바이트 그대로(무손실) 추출됩니다. `false`로 끄면 사진은 항상 원본 그대로 추출되고, PPTX에서 실제로 그룹(Ctrl+G)으로 묶인 사진+도형만 여전히 하나로 합쳐집니다(POI가 그룹을 통째로 그리는 것은 이 옵션과 무관).

**래스터라이즈 한 장의 픽셀 상한** (`PptxImageExtractor.MAX_RASTER_PIXELS`, 1,600만 — 프로퍼티 아님): 위 합성들은 도형 앵커의 **합집합**을 한 장의 `BufferedImage`로 그립니다(`TYPE_INT_RGB`, 픽셀당 4바이트). 그 크기는 슬라이드가 아니라 **앵커**에서 오므로, 슬라이드 밖 멀리 놓인 도형 하나(잘못 저장된 앵커, 캔버스 밖으로 밀어 둔 초안)가 상한 없이는 수 GB 할당이 됩니다 — PDF 내장 이미지(`MAX_IMAGE_PIXELS`)·스캔 PDF OCR 렌더(`MAX_OCR_RENDER_PIXELS`)에 이미 상한이 있던 것과 같은 자리에 뚫려 있던 구멍입니다. 기준 배율은 2배이고, 그대로 그리면 상한을 넘는 경우 면적의 제곱근만큼 배율을 낮춥니다(`renderScale()` — 순수 계산이라 단위 테스트로 고정). **1배 밑으로는 내리지 않고 그리지 않습니다** — 원본보다 작게 그리면 Vision이 읽을 것이 없고, 거기 도달하려면 합집합이 슬라이드의 약 30배 면적이어야 하므로 그건 다이어그램이 아니라 앵커 사고입니다. 16:9 슬라이드 전체(960×540pt)를 2배로 그려도 약 200만 픽셀이라 정상 도형은 배율이 그대로입니다.
