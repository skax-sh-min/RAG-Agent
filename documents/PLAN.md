...
> **재우선순위화**: 실배포 기준(폐쇄망·no-auth 단일 운영자)에서 가치가 없는 **멀티유저(`auth.enabled=true`) 전용 작업**(§6.19·§6.20·§6.16.2·Phase 4)은 전부 후속으로 내렸다(사유는 아래 후속 표의 트리거 열 참조). ⚠️ **§6.19는 이 판단이 절반만 맞았다** — 2026-09-07 실사에서 no-auth·management-only 조합에서만 성립하는 노출면이 여럿 나와 §6.19.4로 처리했다. 남은 것은 §6.19.1(API CSRF)뿐이고 그것은 여전히 인증 모드 전용이다. §6.15(스토리지 쿼터)는 전역 상한이라 단일 운영자에도 적용되므로 이 그룹에 남겨 두었고, 2026-09-01 완료됐다. **§6.25(관리자 대화 목록)도 이름은 §7.3(Phase 4) 계열이지만 같은 이유로 지금 진행 그룹이다** — `app.auth.guest-identity`가 `shared`가 아니면 no-auth 단일 운영자 배포에도 방문자별 대화가 쌓이고, 그중 Step 1은 인증 모드와 무관한 검색 오염 버그 수정이다.
...
### 6.19 보안 하드닝 🟡 §6.19.2·6.19.3·6.19.4 완료 · §6.19.1만 남음

> **범위 주의(작성 당시)**: 아래 3건은 **`app.auth.enabled=true`(멀티유저 인증) 배포에서만** 의미가 있다고 봤다. 폐쇄망 단일 운영자(no-auth) 기본 배포에는 노출면이 없다는 판단이었고, 그래서 §6.20/6.15(쿼터)보다 뒤가 아니라 "인증 켜고 다중 사용자로 열 때 반드시 선행"으로 조건부 우선순위였다.
>
> **그 전제가 절반만 맞았다** — §6.19.3(XFF)은 §6.22에서 IP가 식별자가 되는 순간 no-auth 배포의 선행 필수가 됐고, 2026-09-07 실사에서는 **no-auth·management-only 조합에서만 성립하는** 노출면이 여럿 나왔다(§6.19.4: 게스트 개방 업로드, 공유 게스트 버킷, 무인증 REST 문서 삭제). 인증 모드로 범위를 좁혀 판정한 것 자체가 놓친 원인이다 — **"어느 모드에서 성립하는가"를 항목마다 다시 묻는 편이 낫다.**

**6.19.1 `/api/v1/**` CSRF 비활성 + 세션 인증 혼용 (CSRF 노출)** 🟡 **노출면 축소, 원 항목은 미해결**
- **현재 코드**: `SecurityConfig`가 인증 모드에서 `csrf.ignoringRequestMatchers("/api/v1/**")`로 API 전체를 CSRF 예외 처리하지만, 같은 엔드포인트가 **세션 쿠키 인증**이다. `POST /api/v1/documents`는 `multipart/form-data`(CSRF "simple request")라 크로스사이트 폼으로 인증된 사용자의 브라우저에서 업로드를 유발할 수 있다(`DELETE`·JSON body 경로는 preflight로 상대적으로 안전).
...
- **2026-09-07 진행분(부분)**: 두 가지가 노출면을 줄였지만 **위 완료 기준은 아직 충족하지 않는다**. ① `/api/**`의 CORS 와일드카드 제거 — 크로스 오리진에서 **응답을 읽는** 경로와 프리플라이트가 필요한 `DELETE`는 막혔다. 그러나 `multipart/form-data` POST는 CORS 없이도 전송되는 "simple request"라 이 항목의 핵심 시나리오는 그대로다. ② 문서 쓰기 3종을 `ROLE_ADMIN`으로 게이트(§6.19.2 처리분과 같은 목록) — 피해자가 **관리자일 때만** 성립하도록 좁혔을 뿐, 관리자의 브라우저는 여전히 CSRF 대상이다. 남은 일은 (A)/(B) 중 택일이며, 지금 상태에서는 (B)가 자연스럽다 — 자동화가 이미 로그인 세션을 쓰므로 CSRF 토큰을 요구해도 잃는 것이 적다.
...
- **2026-09-07 보강**: 이 판정의 소비자가 셋이 됐다 — `AuditLogger.extractIp()`가 그때까지 이 플래그를 **보지 않고** `X-Forwarded-For`를 무조건 신뢰하고 있었다. 같은 요청에서 속도 제한·방문자 식별은 실제 주소를 쓰는데 감사 로그만 헤더를 믿었으니, 하필 "누가 무엇을 했는가"를 남기는 것이 목적인 필드가 셋 중 가장 위조하기 쉬웠다. **판정을 한 곳에 모으는 것만으로는 부족하고, 새 소비자가 그것을 실제로 부르는지까지 봐야 한다**는 사례다.

**6.19.4 폐쇄망 배포 실사에서 나온 하드닝** ✅ **완료 2026-09-07**

§6.19의 원래 3건과 별개로, 실제 운영 설정(`AUTH_ENABLED=false`·`AUTH_MANAGEMENT_ONLY=true`·`AUTH_GUEST_IDENTITY=hybrid`·`TRUST_FORWARDED_FOR=false`) 기준으로 코드를 훑어 나온 항목들이다. **"이 설정에 지금 걸리는가"와 "설정을 바꾸면 걸리는가"를 나눠 판정했고, 후자도 함께 고쳤다** — 기본값으로 되돌리는 순간 노출되는 것을 남겨 두면 설정 변경이 곧 사고가 된다.

- **`/api/**` CORS 와일드카드 제거**: 인증 없는 두 모드에서 그 접두사는 `permitAll` + CSRF 예외라, 크로스 오리진 허용이 곧 코퍼스 열람 권한이었다. 소비자가 전부 same-origin 이고 스크립트는 CORS 를 보지 않아 **잃는 기능이 없다**.
- **management-only 모드의 REST 문서 쓰기 게이트**: `POST /api/v1/documents`·`/sync`·`DELETE`가 무인증으로 열려 있어, 그 모드를 켜는 이유인 UI 게이트가 장식이었다(`curl -X DELETE` 한 줄이 우회). 문서상 "curl 자동화 보존"을 위한 의도적 결정이었으나, 자동화는 `/login` 세션 쿠키로 그대로 동작하므로 없어지는 것은 기능이 아니라 지름길뿐이다.
- **full-auth 모드의 문서 관리 RBAC**: 그 모드에만 문서 게이트가 없어 `/signup`이 열린 배포에서는 **가입만 하면** 문서를 갈아치울 수 있었다. 두 모드가 `gateDocumentManagement()` 하나를 공유하도록 합쳤다(이번 구멍이 정확히 "한쪽에만 있어서" 생겼다). **짝으로 `/setup`을 모드 무관하게 열었다** — `createAdminUser()`를 부르는 곳이 거기뿐이라 그전까지 full-auth 모드에는 관리자를 만들 방법이 아예 없었고, `/admin/**`은 이미 아무도 못 여는 문이었다.
- **공유 게스트의 레이트리밋 키**: `guest-identity=shared`(기본)에서 전 방문자가 버킷 하나를 나눠 써, 한 명이 한도를 소진하면 나머지 전원이 429였다 — 남용을 막는 장치가 그 자체로 서비스 거부의 지렛대. 그 id 일 때만 IP 로 떨어뜨린다(`hybrid` 등은 이미 방문자별이라 무영향).
- **게스트 개방 이미지 업로드**: `POST /curated/submissions/images`가 `default` 버킷(120/분)이라 5MB × 120 = 분당 600MB를 밀어 넣을 수 있었고 저장 상한은 기본 무제한이다. `upload` 버킷(10/분)으로 옮기고, 기동 시에만 돌던 고아 정리를 **6시간 주기**로 바꿨다(`@EnableScheduling` 도입 — 앱의 유일한 `@Scheduled`).
- **로그·세션 위생 3건**: 감사 로그 IP(위 §6.19.3 보강) · `X-Trace-Id` 모양 검사 · `ThreadContextResolver`의 세션 생성 중단(REST 호출마다 8시간 세션이 쌓이고 있었다).
- **컨테이너 비-root(UID 10001)**: 신뢰 경계 밖 문서를 파싱하는 프로세스라, 그중 하나에서 코드 실행이 일어났을 때 root 인지가 그다음을 가른다. ⚠️ 기존 Linux 배포는 `chown -R 10001:10001 ./data` 1회 필요 — 엔트리포인트가 쓰기 권한을 먼저 확인해 그 명령을 안내하고 종료한다.

**유지해야 할 결정**
- 문서 관리 게이트 목록은 **두 인증 모드가 공유하는 하나**(`SecurityConfig.gateDocumentManagement`)다. 한쪽에만 추가하면 이번과 같은 구멍이 다시 난다.
- REST 쓰기 3종은 `NoAuthAutoLoginFilter.isGatedManagementPath()`에 **일부러 넣지 않았다** — 게스트 principal 이 주입된 채 `hasRole("ADMIN")`에서 막혀 302(로그인 페이지)가 아니라 **403**이 나가고, 그게 API 호출자에게 맞는 응답이다.
- `UPLOAD_MAX_TOTAL_SIZE`의 기본 무제한은 그대로 뒀다(문서화된 결정). 게스트 개방 배포에서는 운영자가 설정하는 것이 방어선이며, 코드 기본값을 바꾸면 기존 배포가 조용히 업로드를 거부하게 된다.
