...
>
> **2026-09-01 추가 완료**: §6.26 컨텍스트 초과 대응(창 인지 + 입력 예산, 8단계 — 답변·검증에 이어 인덱싱 재작성까지) · §6.24 의 남은 이슈였던 토큰 추정 4배 불일치 해소 · C(응용) 모드 운영자 스위치(`app.llm.creative-mode-enabled`) · `/settings` 허용 범위 툴팁과 인덱싱 3종 범위 조정 · `/admin` 문서 레지스트리 페이지네이션 · §6.15 전역 저장 상한(`app.upload.max-total-size`).
...
**9단계로 나눠 진행했다** — 각 단계가 단독으로 가치가 있고 뒤 단계의 전제가 된다.
...
- **`ContextWindowProbe` 는 기동 시 1회다.** 서버가 다른 컨텍스트로 모델을 다시 로드하면 값이 낡는다 — `/settings` 가 값과 출처(설정됨/탐지됨)를 함께 보여주는 이유이고, 어긋나면 운영자가 `context-size` 로 못 박으면 된다.
