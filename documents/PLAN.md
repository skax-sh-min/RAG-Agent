...
| 1 | **§10.12 짧은 후속 질문의 독립화** — 검색 축 셋이 전부 질문 원문을 본다. `"그 설정은 어디야?"` 같은 짧은 후속 질문은 검색어로 무의미한데, 하필 그 길이 구간이 MultiQuery 확장에서도 제외된다 | 미착수(설계 완료) |
| 2 | **§10.13 Direct 턴의 이력 확대** — Direct 프롬프트에는 `[검색된 문서]` 블록이 통째로 없는데 이력 상한은 모드와 무관하게 5,000자로 고정이라, 이력만으로 답해야 하는 모드가 이력을 가장 적게 받는다 | 미착수(설계 완료) |
...
> **§10.11 · §10.12 · §10.13 구현 순서 — `10.11 → 10.13 → 10.12`** (§10.11 완료, 다음은 §10.13)
>
