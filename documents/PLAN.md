...
> **2026-09-02 추가 완료**: §6.27 검증 실패 재시도 개선(피드백 루프 · 게이트별 분기 · 청크 교체 · 여유 기반 증가) · QA 픽스 6건(저장 상한이 삭제로 회수되지 않던 문제, 다중 파일 업로드 429, 업로드 거부 사유 유실, 임시 파일 누수 등).
>
...
| 순위 | 항목 | 현재 상태 |
|---|---|---|
| 1 | **§10.11 좋아요 → 지식 제안 경유** — 검색 코퍼스로 들어가는 문을 하나로. 좋아요가 지금은 무검토로 `curated_qa` 에 직행해, 문서를 하나도 안 본 Direct 답변까지 3초 뒤 전체 검색 지식이 된다 | 미착수 |
| 2 | **§10.12 짧은 후속 질문의 독립화** — 검색 축 셋이 전부 질문 원문을 본다. `"그 설정은 어디야?"` 같은 짧은 후속 질문은 검색어로 무의미한데, 하필 그 길이 구간이 MultiQuery 확장에서도 제외된다 | 미착수(설계 완료) |
| 3 | **§10.13 Direct 턴의 이력 확대** — Direct 프롬프트에는 `[검색된 문서]` 블록이 통째로 없는데 이력 상한은 모드와 무관하게 5,000자로 고정이라, 이력만으로 답해야 하는 모드가 이력을 가장 적게 받는다 | 미착수(설계 완료) |
...
### 10.10 좋아요 기반 큐레이션 Q&A 지식화 ✅ ①②③④ 전체 완료 (유입 경로는 §10.11 로 대체 예정)
...
> **§10.11 · §10.12 · §10.13 구현 순서 — `10.11 → 10.13 → 10.12`**
>
...
### 10.11 좋아요 → 지식 제안 경유 (§10.10 유입 경로 대체) 🔵 미착수

§10.10 은 좋아요를 누르면 `curated_qa` 행을 **동기적으로** 만들고 3초 뒤 임베딩해 전체 사용자의 검색에 넣는다. 그 결과 지금 검색 코퍼스로 들어가는 문이 **둘인데 경비가 정반대**다.

| 경로 | 게이트 | 반영 |
|---|---|---|
| 청크 추가 제안 | 관리자 승인 필수 | 승인 시 |
| **좋아요** | **없음** | **3초 후** |

CLAUDE.md 는 제안 게시판에 대해 *"관리자 승인이 사용자 텍스트와 답변 프롬프트의 `[검색된 문서]` 블록 사이의 유일한 게이트"* 라고 못 박아 두었는데, 좋아요가 그 문장을 조용히 거짓으로 만들고 있다. **Direct(문서를 하나도 안 본) 답변이 좋아요 한 번에 검색 지식이 되는 것**이 그 구멍의 가장 날카로운 모양이다 — `onLike()` 는 `allowsCuration()` 만 보고 `direct_mode` 를 확인하지 않는다.

**해결 방향은 `direct_mode` 가드를 코드에 박는 것이 아니라 문을 하나로 모으는 것이다.** 판단이 필요한 일에는 판단을 넣는다 — 관리자가 검토 화면에서 `[DN]` 표기(§ 질문 버블 두 글자 표기)를 보고 반려하면 된다.

**확정된 모델 — 책임 분리**

- **채팅** = 감상. 좋아요는 `"이 답변이 좋았다"` 그 이상도 이하도 아니다. 큐레이션 상태를 일절 표시하지 않는다(상태 칩 없음, **연필 버튼 제거**, 임베딩 실패 배지 제거).
- **지식 제안 페이지** = 지식 관리. 등록·수정·철회가 전부 여기서만 일어난다.
- 좋아요 클릭 → `"지식 제안으로 등록하시겠습니까?"` → **예**: 좋아요 기록 + 제안 페이지로(프리필) / **아니오**: 좋아요 취소.

**정책 변경 3건 (§10.10 에서 뒤집힌 것)**

1. **큐레이션 검색축을 하나로 합친다.** §10.10 은 `MetaKey.CURATED_ORIGIN` 으로 `like`/`manual` 두 축을 나눠 가중치를 따로 주었다. 그 구분의 근거는 *"앱이 만든 미편집·무검토 출력"* 대 *"사람이 쓴 텍스트"* 였는데, 이제 **모든 유입이 사람 편집 + 관리자 승인을 거치므로 그 차이가 사라진다.** `app.search-curated-qa-weight` 하나만 남기고 `app.search-submission-weight` 를 제거한다(`RetrievalService` 의 파티션 분기와 `/settings` 행도 함께). `CURATED_ORIGIN` 메타데이터 자체는 계속 쓴다 — 감사·통계용이며 검색 분기에서만 빠진다.
2. **C 는 열고 S 는 계속 막는다.** §6.24 가 C 를 큐레이션에서 뺀 이유는 *"모델이 지어낸 코드가 다음 턴의 문서가 된다"* 였는데, 그건 **게이트가 없었기 때문**에 치명적이었다. 사람이 편집하고 관리자가 승인하는 경로에서는 C 답변도 다른 제안과 같은 심사를 받는다.
   - **S 는 제외를 유지하되 그 사유가 바뀐다.** 지금 코드·툴팁이 대는 이유(*"구조 섹션을 걷어내면 남는 것이 없다"*, `feedback.like.disabled.s`)는 이 경로에서 **더 이상 성립하지 않는다** — 본문이 사람이 편집한 텍스트라 스트립 전제가 사라진다. 실제 사유는 **S 가 의도적으로 축약된 답변**이라는 것이다: 1,000자 상한의 `## 요약` 한 섹션이고, 프롬프트가 배경·이유·전제·예외를 **빼라고 지시**한다. 지금 짧게 보고 싶어서 고른 형식이지 오래 남길 지식의 원본이 아니다. **사유를 함께 고치지 않으면** 나중에 코드를 읽는 사람이 "이 근거는 틀렸다"며 S 를 열게 된다.
   - 따라서 `allowsCuration()` 은 남지만 뜻이 바뀐다 — *"좋아요가 지식 제안을 만들 수 있는가"*. 이름(`allowsSubmission()` 등)·주석·`curationBlockedMessageKey()` 의 S 문구를 함께 고치고, `feedback.like.disabled.c` 는 한/영 모두 제거한다.
3. **승인된 제안을 수정해도 기존 등록본은 검색에 남는다.** 수정하면 제안이 `pending` 으로 돌아가되 `curated_qa` 행은 활성 그대로이고, 관리자가 승인할 때 **기존 행을 내리고 새로 만든다**(교체). 오타 하나 고치는 동안 그 지식이 며칠 사라지는 것은 과하다. **새 컬럼도 새 상태도 필요 없다** — `displayStatus()` 가 `approved + 활성행 0 → 회수됨` 으로 파생하므로 `pending` 인데 활성 행이 있으면 그냥 `pending` 으로 읽힌다(목록에 `검토 대기(수정본) · 현재 등록본은 계속 사용 중` 으로 표기).

**지식 제안 페이지에 오늘 없는 기능 둘** — 이 모델의 전제이므로 반드시 만들어야 한다.

- **철회가 `pending` 일 때만 된다.** 템플릿이 `th:if="${s.isPending()}"` 이고 `markWithdrawn` 의 CAS 도 `status='pending'` 조건이라, **등록 완료된 제안을 저자가 내릴 방법이 없다**(관리자 회수뿐). `forceRemoveBySubmission()` + `approved → withdrawn` CAS 를 추가한다.
- **수정 경로가 아예 없다.** pending 은 저자가 직접 수정(CAS `status='pending'`), approved 는 위 정책 3.

**단계**

1. **서버 기반** — `curated_submission` 에 `source_turn_id`/`source_thread_id` 추가(방어적 `ALTER`, `tags` 선례). 프리필 조회 `GET /curated/submissions?fromThread=&fromTurn=`.
2. **승인 분기** — 좋아요 출신은 `insertManual()` 이 아니라 좋아요 경로로 등록(아래 함정 ①②).
3. **제안 페이지** (가장 큰 덩어리) — 저자 수정 · 승인분 철회 · 상태 필터 · 전문 편집 오프캔버스(`/admin` 검토 오프캔버스와 같은 컴포넌트) · 중복 제안 방지 · pending 상한 예외.
4. **채팅 UI** — 확인 대화상자(기존 싫어요 확인과 같은 패턴) + `PATCH feedback` + 프리필 이동, 연필·배지 제거. **렌더러 셋 전부**(기록 루프·스트리밍·재사용).
5. **직접 등록 제거 + 관리자 화면** — `onLike()` 의 `upsertActive`+임베딩 스레드 제거, 검토 화면에 `[RN]`/`[DN]` 표기와 원 대화 링크. **마지막에 두는 이유**: 먼저 지우면 새 경로가 완성될 때까지 큐레이션이 통째로 멈춘다.

**조용히 깨지는 지점 (코드를 읽고 찾은 것)**

- **① 저장 모양이 다르다.** 제안은 `행 N개`(`splitBody` 가 미리 자름), 좋아요는 `행 1개 → 벡터 N개`(`splitForEmbedding` 이 임베딩 시점에 자름). 좋아요가 행을 안 쪼개는 이유는 `UNIQUE(source_turn_id)`·취소·인라인 편집이 **전부 turn 단위**이기 때문이라, `insertManual` 을 태우면 `source_turn_id` 가 `NULL` 로 박히고 그 셋이 함께 죽는다.
- **② `upsertActive()` 에 `source_submission_id` 를 실어야 한다.** `Submission.displayStatus()` 가 그 컬럼으로 LEFT JOIN 해 활성 행을 세는데, 비어 있으면 **승인 직후 "회수됨"** 으로 표시된다. 오류도 로그도 없다.
- **③ 프리필 본문은 반드시 서버가 턴에서 읽는다.** URL·hidden 필드로 나르면 3,000자 답변이 길이를 넘고, 무엇보다 누구나 임의 텍스트를 *"채팅 답변에서 온 것"* 으로 위장해 제출할 수 있다. 제목은 질문을 `MAX_TITLE_LEN`(200)으로 **잘라야** 한다 — 질문은 2,000자까지 가능해서 안 자르면 폼이 예외로 죽는다.
- **④ `MAX_PENDING_PER_USER=20` 이 게스트 공유 모드에서 전 방문자 하나다**(`guest-identity=shared`, 기본값). 좋아요가 이 상한을 함께 쓰면 20건에서 **조용히 실패**한다 — 손으로 쓴 글은 폼에 오류를 띄우면 되지만 좋아요는 버튼 한 번이라 설명될 자리가 없다. 좋아요 출신은 상한에서 뺀다(남용은 중복 제안 방지가 막는다).
- **⑤ 연필 버튼 조건.** [chat.html] 의 인라인 편집이 `feedback=='LIKE' and curatable` 로 뜨는데 승인 전에는 편집할 행이 없다. 이 모델에서는 아예 제거한다 — 바로 위 주석이 *"연필만 보고 누르면 편집할 대상이 없다"* 는 **똑같은 사고를 이미 한 번** 적어 두었다.
- **⑥ `validateImageCount()` 는 마커 전체를 센다**(문서 이미지 포함, 상한 10). 이미지 많은 답변은 제출에서 걸리므로 프리필 화면이 개수·상한을 **미리** 보여야 한다. 이미지 삭제 자체는 안전하다 — `OWNED_IMAGE_PATH` 가 `images/submissions/` 로 한정돼 문서 이미지는 건드리지 않는다.
- **⑦ `LIKE` 의 소비자가 큐레이션 하나가 아니다.** `QuestionReuseRepository` 의 `AND (COALESCE(t.direct_mode,0)=0 OR t.feedback='LIKE')` 때문에 **Direct 턴은 좋아요가 있어야 재사용된다**. `아니오 = 좋아요 취소` 라 "제안은 싫지만 답변은 좋았다" 를 표현할 수 없고, 그런 Direct 답변은 재사용 대상에서 빠진다(수용). CLAUDE.md 의 `"LIKE's only consumer is curation"` 은 이 조항 때문에 부정확하므로 함께 고친다.

**하지 않는 것**

- 기존 `origin='like'` 활성 행 마이그레이션 없음 — 이미 `/admin` 에서 보이고 회수된다. 축을 합치면 그 행들도 단일 가중치로 검색된다.
- **S 를 여는 것** — 위 정책 2 참고. 축약이 목적인 답변을 지식 원본으로 삼지 않는다.
- **`allowsCuration()` 을 없애는 것** — S 하나가 남으므로 판정 자체는 계속 필요하다(값이 아니라 성질로 분기하는 §6.24 규약도 그대로).

**열린 항목**

- **(a) 중복 제안 방지의 구현 위치.** 같은 턴에 pending/approved 제안이 이미 있으면 새로 만들지 않고 그 항목으로 보낸다. `curated_qa` 의 `UNIQUE(source_turn_id)` 는 승인 후에만 걸리므로 그 앞 단계를 서비스에서 막아야 한다(부분 UNIQUE 인덱스도 가능하지만 상태 전이가 얽힌다).
- §10.10 의 열린 항목 (a)(b)는 그대로 유효하다.

---

### 10.12 짧은 후속 질문의 독립화 (condense) 🔵 미착수 — 설계만

**문제.** 검색 축 셋(벡터 · BM25/FTS · 큐레이션)은 전부 **질문 원문**을 그대로 본다. 이력은 답변 생성 단계에서만 쓰이므로, `"그 설정은 어디에 있어?"` 같은 후속 질문은 검색어로서 의미가 없어 엉뚱한 문서가 올라온다. 사용자에게는 "문서에 있는데 못 찾는다"로 보인다.

**하필 그런 질문이 확장도 못 받는다.** `app.search-multiquery-min-length`(기본 15자) 미만이면 MultiQuery 확장을 통째로 건너뛴다(`RetrievalService.shouldExpand()`). 그런데 **짧은 질문이 곧 맥락 의존적인 질문**이다 — `"그거 더 자세히"`(9자), `"그 설정은 어디야?"`(10자). 지금은 **확장도 없고 맥락도 없는** 최악의 조합이 정확히 이 구간에 몰려 있다.

**이력을 분류기·MultiQuery 에 직접 넘기지 않기로 했다** (2026-09-02 결정). 두 가지 이유다.

- **분류기**: 분류 결과 5개(concept/usage/error/version/meta) 중 흐름을 바꾸는 것은 `meta` 하나뿐이다(`AgentGraph` 의 검색 스킵). 그 한 갈래를 위해 매 턴 이력 전체를 5지선다에 태우는 것은 값이 맞지 않고, `AgentService.chat()` 이 지금 **이력 로딩과 분류를 병렬로** 돌리는 구조도 깨진다(분류가 이력을 기다리게 된다).
- **MultiQuery**: 넣어도 **벡터 축만** 고쳐진다. BM25/FTS 와 큐레이션 축은 여전히 질문 원문을 토크나이즈하므로 절반은 그대로 틀린 채 남는다.

**대신 질문을 다시 쓴다.** 이력 + 후속 질문 → 자립적인 질문 하나로 재작성하고, 그 결과를 **분류기 · MultiQuery · FTS 가 모두** 쓴다. 한 번의 재작성이 세 축을 동시에 고치고, 분류기 문제(`"왜?"` 가 잡담으로 읽히는 경우)도 같은 값으로 해결된다 — 분류기에 필요한 것은 이력이 아니라 **독립화된 질문**이었다.

**게이트는 이미 있는 것을 쓴다 — LLM 호출 순증 0.**

```
질문 길이 < search-multiquery-min-length  AND  이력 있음
    → MultiQuery 확장 대신 독립화(condense)
그 외
    → 지금 그대로
```

이미 **건너뛰던** 호출 자리를 쓰는 것이라 턴당 호출 수가 늘지 않는다. 긴 질문은 대체로 자립적이므로 지금처럼 확장만 한다. 라우팅은 `TaskType.MICRO_TEXT` — 쿼리 다양화·키워드 추출·제목 생성과 같은 "추론 불필요 잡무" 계층이고, 소형 프로바이더를 등록한 배포라면 큰 모델을 전혀 건드리지 않는다(§6.21).

**설계 시 지켜야 할 것**

1. **독립화된 질문은 검색에만 쓰고, 답변 프롬프트의 `[현재 질문]` 은 원문을 유지한다.** 사용자가 실제로 쓴 말이 답변의 어조와 초점을 정한다. 재작성이 빗나가도 검색만 틀리고 답변까지 함께 틀어지지는 않게 하는 격리이기도 하다.
2. **이미 자립적이면 그대로 돌려주도록 프롬프트에 명시한다.** `"SSE 타임아웃?"`(10자)처럼 **짧지만 자립적인** 질문이 이력에 오염되는 것이 이 기능의 주된 실패 모드다. 길이는 맥락 의존성의 대리 지표일 뿐이라 오탐이 있고, 그 오탐을 흡수하는 것은 프롬프트뿐이다.
3. **재작성 결과를 화면에 보여야 한다.** 잘못된 재작성은 사용자에게 "나쁜 검색어"가 아니라 **"엉뚱한 답변"** 으로만 보여 원인을 짚을 수 없다. `ui.retrieval-metrics-enabled` 진단 패널에 한 줄이면 되고, 로그에도 남긴다. 이 앱이 축소(`budgetNote`)·미사용 출처·`envNote` 에서 반복해 온 원칙과 같다 — **모델이 본 것과 사용자가 본 것이 다르면 말한다.**

**검토했으나 고르지 않은 것**

- **직전 턴의 질문을 검색 텍스트 앞에 붙이기**(LLM 호출 0). 구현이 몇 줄이고 공짜지만, 주제가 바뀐 후속 질문에서는 그대로 노이즈가 된다 — 이 앱의 검색은 이미 RRF 로 세 축을 합치므로 한 축에 들어간 노이즈가 조용히 순위를 흔든다. 정확도 차이가 커서 제외했다.
- **답변까지 포함해 재작성하기.** 직전 답변은 2~3천 자라 MICRO_TEXT 계층에 넘기기에 무겁고, 답변에 등장한 표현이 질문에 섞여 들어가 **검색이 자기 답변을 다시 찾는** 쪽으로 기운다(§10.11 이 큐레이션에서 경계하는 것과 같은 모양의 순환).

**열린 항목**

- **(a) 길이 임계값을 그대로 쓸 것인가.** `search-multiquery-min-length` 는 "확장할 가치가 있는가"를 재는 값이지 "맥락 의존적인가"를 재는 값이 아니다. 지금은 두 판정이 우연히 같은 방향이라 재활용하지만, 오탐이 잦으면 별도 임계값이 필요하다 — 그때 **핫 편집 키를 하나 더 늘릴지**를 함께 판단할 것.
- **(b) 이전 턴이 Direct 였을 때의 오염.** 독립화의 재료는 이력인데, DN 답변은 **문서에 근거하지 않은 텍스트**다. `DN → RN` 순서에서 모델이 지어낸 용어가 재작성된 검색어에 섞여 들어가면, 검색은 없는 것을 찾고 RN 답변은 나쁜 검색 위에 세워진다 — 게다가 그 답변이 다시 이력이 되어 같은 용어를 굳힌다. 재료를 `## 요약` 이 있는 턴(= 검색을 거친 답변)으로 제한하거나, 최소한 진단 패널에서 재작성의 출처 턴을 함께 보여줄 것. §10.13 과 **함께 구현될 때 특히 커진다**(그쪽이 Direct 답변을 이력에 더 많이 넣는다).
- **(c) 재작성 실패 시의 폴백.** 파싱 실패·빈 응답이면 **원문으로 검색한다**(지금 동작). 재작성이 없다고 검색을 멈출 이유는 없고, 이 앱이 `withoutVerdict()`·`ProviderContextWindows` 에서 지켜 온 "모르면 아무것도 하지 않는다"와 같은 규칙이다.

---

### 10.13 Direct 턴의 이력 확대 — 문서 자리를 이력에 돌려준다 🔵 미착수 — 설계만

**문제.** Direct(비검색) 답변의 프롬프트에는 `[검색된 문서]` 블록이 **통째로 없다**. 기본 설정에서 그 자리는 `topK 10 × chunk-size 1,500` ≈ 15,000자인데, 이력 상한은 모드와 무관하게 `maxConversationChars() = max(1000, LLM_MAX_TOKENS/2)` = 5,000자로 고정이다. 창의 큰 부분이 놀고 있고, 정작 이력만으로 답해야 하는 모드가 이력을 가장 적게 받는다. 그중에서도 **DN**(Direct + N)이 정확히 그 칸이다 — `## 요약` 이 없어 `RECENT_DIRECT_ANSWER_CAP`(1,200자)에 실제로 걸리는 유일한 종류이고(RAG 턴은 요약 섹션만 들어가 캡에 닿지 않는다), 형식 지시 없이 *"구체적이고 자세하게"* 쓰라고 지시받은 답변이라 원문이 가장 길다.

측정하면(창 40,960 · N 스트리밍 출력 예약 5,000 · 여유 4,096):

| | 문서가 가져가는 자리 | 이력에 줄 수 있는 자리 | 지금 주는 양 |
|---|---|---|---|
| RAG | ~15,000자 | ~16,000자 | 5,000자 |
| **Direct** | **0** | **~31,000자** | **5,000자** |

**규칙은 "Direct라서"가 아니라 "문서 자리가 비어서"로 잡는다.**

```
이력 상한 = 입력 예산 − 문서가 차지할 자리
```

모드별 상수를 두 개 만들지 않는다. 이렇게 두면 나중에 `topK` 를 낮춘 배포나 검색 결과가 적게 나온 턴에도 같은 논리가 그대로 적용된다 — 모드로 분기하면 그 일반화가 막힌다.

**다만 로딩 시점에 그 자리를 확실히 아는 것은 Direct 뿐이다.** 이력은 검색보다 **먼저** 로딩된다(`AgentService.chat()` 이 이력 로딩과 분류를 병렬로 돌린다). 그 시점에 문서가 실제로 몇 개 올지는 모르고, Direct 만 검색이 아예 돌지 않아 0 이 확정이다. RAG 쪽 불확실성은 검색이 끝난 뒤 도는 `AnswerService.fitToBudget()` 이 이미 담당하므로, **1차 적용은 Direct 에만** 한다. RAG 는 `topK × chunk-size` 추정으로 넓힐 수도 있으나 그건 별도 판단이다(열린 항목 (a)).

**이전 턴에서 무엇을 넣을지 — 두 축이 동시에 관여한다**

헷갈리기 쉬운 지점이라 먼저 못 박는다. **지금 묻는 턴이 Direct 인가**가 *얼마나* 넣을지를 정하고, **이전 턴이 어떤 모드였나**가 *무엇을* 넣을지를 정한다. 서로 다른 턴의 서로 다른 축이다.

| 이전 턴의 성격 | `[Recent]` 에 넣는 것 |
|---|---|
| **S** (RS · DS) | `## 요약` 섹션 — 그게 답변 전부다 |
| **N · C** (RN · RC · **DN**) | `## 요약` 과 `## 참고` 를 **뺀** 나머지 |

**`## 요약` 의 유무는 응답 모드 혼자 정하지 않는다 — 검색 축도 관여한다.** `prompt.answer.system.*`(RAG)와
`prompt.direct.system.s`(DS)는 형식으로 `## 요약` 을 지시하지만, `prompt.direct.system.n`(DN)은
*"답변 구조는 질문의 성격에 맞게 자유롭게 선택"* 이라 **형식 지시가 없다** — DN 답변에는 대체로 `## 요약` 이 없다.

그래도 위 두 갈래로 충분하다. DN 은 두 번째 갈래로 떨어지는데 **뺄 섹션이 애초에 없어** 전문이 그대로 남고,
`## 참고` 제거도 검색이 없으니 무의미하다. 즉 **DN 의 순효과는 캡 제거뿐**이며, 그것이 이 항목이 노리는 바다.

**이 렌더링 규칙 전체가 "지금 묻는 턴이 Direct 일 때" 로 스코프된다** — 예산도, 캡 제거도, 요약 제외도 전부. RAG 로 물을 때는 지금 동작 그대로다(문서가 자리를 가져가므로 넓힐 여유가 없다). 한 스레드에 두 축이 섞이므로 조합으로 적어 둔다:

| 이전 턴 → **지금 묻는 턴** | 이전 답변에서 `[Recent]` 에 들어가는 것 |
|---|---|
| DN → **Direct** | **전문**(캡 없음) + 넓은 예산 — 아래 1단계를 적용하면 이 행도 `## 요약`·`## 참고` 를 뺀 나머지가 되어 두 번째 행과 합쳐진다 |
| RN · RC → **Direct** | `## 요약`·`## 참고` 를 뺀 **본문** + 넓은 예산 |
| RS · DS → **Direct** | `## 요약`(= 답변 전부) + 넓은 예산 |
| 무엇이든 → **RAG** | **지금 그대로** (RAG 턴은 `## 요약` 만, DN 턴은 1,200자 캡) |

**같은 이전 턴이 다음에 무엇을 묻느냐에 따라 다르게 렌더된다.** `RN → Direct` 면 본문이 통째로, `RN → RAG` 면 요약 두세 문장만 들어간다. 근거는 자의적이지 않다 — 이번 턴에 무엇이 함께 들어가야 하는지가 다르고, `fitToBudget()` 이 검색 문서를 턴마다 다시 재는 것과 같은 논리다. 다만 사용자에게는 *"아까는 기억했는데 왜 지금은 못 하지?"* 로 보이므로 열린 항목 (b)의 노출이 여기서 특히 중요하다.

**밀려난 뒤의 수명도 다르다.** `[Recent]` 창(기본 2턴) 밖으로 밀려나면 요약 블록으로 넘어가는데, `## 요약` 이 없는 DN 답변은 거기서 `UNSUMMARIZED_ANSWER_CAP`(300자)로 잘린다(MICRO_TEXT 오프로드 프로바이더가 없는 일반 배포 기준. 있으면 LLM 이 요약한다). 즉 DN 답변은 **전문 → 1,200자 → 300자** 로 늙는다.

- **S 를 특별 취급하는 근거는 프롬프트에 있다** — `prompt.answer.system.s` 가 *"출력 전체를 `## 요약` 헤더 한 줄로 시작... 다른 `##` 헤더를 만들지 말고, 요약 외의 어떤 내용도 덧붙이지 마세요"* 라고 지시한다. 요약을 빼면 남는 것이 없다.
- **N·C 에서 `## 요약` 은 본문의 재진술**이라, 본문을 넣을 거면 같은 내용을 두 번 싣는 셈이다.
- **`## 참고`(인용 목록)도 뺀다** — 검색이 돌지 않는 Direct 후속 턴에서 출처 목록은 값이 거의 없고 길이는 길다. 반면 C 의 **`## 검증되지 않은 부분` 은 남긴다**: 다음 턴이 "무엇이 확인되지 않았는지" 를 아는 것은 실제로 쓸모가 있다.
- **분기는 값이 아니라 성질로 한다.** `mode == ResponseMode.S` 는 `ResponseModeBranchConventionTest` 가 빌드 실패로 막는다. 마침 `ResponseMode.summaryOnly()` 가 정확히 S 만 참이므로(enum 인자 확인: S `..., false, true, ...` / N `..., true, false, ...`) 새 메서드 없이 그대로 쓴다.

**1단계로 `DN` 도 `## 요약` 을 내게 한다 — 나머지가 전부 단순해진다**

`prompt.direct.system.n` 은 지금 *"답변 구조는 질문의 성격에 맞게 자유롭게 선택"* 이라 형식 지시가 없고, 그래서 DN 답변에는 `## 요약` 이 없다. 여기에 요약 섹션을 (아래 조건부로) 추가하면 이 항목 밖에서도 이득이 나온다:

- **요약 경로의 LLM 호출이 대부분 사라진다.** `buildSummaryInput()` 은 각 답변을 자기 `## 요약` 으로 치환하고 **전부 있으면 LLM 을 아예 안 부른다**(`fullyPreSummarized`). 지금 그 조건을 깨는 유일한 원인이 `## 요약` 없는 DN·meta 답변이다.
- **`UNSUMMARIZED_ANSWER_CAP`(300자)의 거친 절단이 사라진다.** MICRO_TEXT 오프로드 프로바이더가 없는 일반 배포에서 DN 답변은 지금 300자로 뭉텅 잘린다.
- **`RECENT_DIRECT_ANSWER_CAP`(1,200자)이 정상 경로에서 빠진다.** `DN → RAG` 에서도 다른 N 턴처럼 요약이 들어가므로, 임의의 글자 수로 자를 이유가 없어진다. RAG↔Direct 를 자주 오가도 렌더링이 균일해진다.

**단, 강제하지 않는다 — 분량 조건부로 넣는다.** `prompt.direct.system.n` 은 **meta 분류 답변도 함께 쓴다**(RAG 모드에서 "안녕하세요"가 `DirectAnswerService` 를 탄다). 인사말에 `## 요약` 헤더가 붙으면 안 된다. 강제하지 않아도 되는 근거는 **폴백이 이미 옳다**는 것이다 — 모델이 형식을 안 따르거나 답변이 짧아 요약이 없으면 위 규칙이 "뺄 게 없으니 전문"으로 떨어지고, 짧은 답변에는 그게 정답이다. 순응 실패와 짧은 meta 답변이 **같은 폴백으로 안전하게 수렴**한다.

`RECENT_DIRECT_ANSWER_CAP` 은 **폴백으로 남긴다** — 요약 없이 길게 나온 답변의 안전판이다. 정상 경로에서 빠질 뿐 삭제하지 않는다.

**조용히 깨지는 지점**

- **① `RECENT_DIRECT_ANSWER_CAP`(1,200자) 제거만 먼저 하면 회귀다.** `[Recent]` 는 최신 턴부터 역순으로 채우다 예산을 넘기면 멈춘다. 캡만 빼면 N 답변 본문 하나(2~3천 자)가 5,000자를 거의 다 먹어, Direct 턴이 **지금(1,200×2)보다 적은 수의 턴**을 보게 된다. 캡 제거와 예산 인상은 한 커밋이어야 한다.
- **② 두 이력 경로가 이미 다르게 동작한다.** 요약 경로(`ConversationSummarizerService.buildContext()`)는 RAG→`## 요약`만 / Direct→1,200자 캡인데, 폴백 경로(`SqliteMemoryRepository.getHistory()`)는 **전문 그대로** 넣는다(전체 문자 예산만 걸린다). 같은 스레드가 **요약 캐시가 있느냐에 따라 다른 맥락을 본다.** 이 작업에서 두 경로를 같은 규칙으로 맞추지 않으면, 캐시 TTL 이 지난 뒤 이력이 갑자기 달라지는 재현성 문제가 남는다.
- **③ `DirectAnswerService` 에는 안전망이 없다.** `fitToBudget()` 을 한 번도 호출하지 않는다(grep 0회). 지금은 5,000자 고정이라 문제가 없지만, 상한을 올리면 창을 넘길 수 있다. **Direct 경로도 `fitToBudget()` 을 타야 한다.**
- **④ `maxConversationChars()` 는 §6.11 이 단일 진실 소스로 못 박은 값이다.** 모드/자리 인지로 바꾸면 요약 경로와 폴백 경로가 **둘 다** 같은 새 규칙을 읽어야 한다. 한쪽만 고치면 ②가 그대로 재발한다.

**하지 않는 것**

- **`RECENT_DIRECT_ANSWER_CAP` 자체를 키우는 것** — Direct 답변은 문서에 근거하지 않은 텍스트라, 더 많이 넣으면 그 위에 또 쌓여 오류가 대화 내내 증폭된다(§6.24 가 C 를 큐레이션에서 뺀 것과 같은 종류의 우려). **턴 수는 늘리되 턴당 캡은 유지**한다.
- **`[Recent]` 의 RAG 턴을 전문으로 바꾸는 것** — 답변이 2~3천 자라 한 턴이 예산을 통째로 먹는다. RAG 턴은 지금처럼 요약 섹션만으로 충분하다.

**열린 항목**

- **(a) RAG 턴에도 같은 계산을 적용할 것인가.** 로딩 시점에는 `topK × chunk-size` 상한으로 추정할 수밖에 없고, 실제 검색 결과는 그보다 적은 것이 보통이다. 과대 추정하면 이력이 줄어 지금보다 나빠지므로, 보수적으로 잡거나 아예 손대지 않는 편이 낫다.
- **(b) 늘어난 사실을 사용자에게 보일 것인가.** 한 스레드에 RAG·Direct 턴이 섞이므로 같은 대화가 턴마다 다른 양의 이력을 본다 — *"아까는 기억했는데 왜 지금은 못 하지?"* 가 나온다. 축소를 알리는 자리(`budgetNote`)가 이미 있으니 확장도 같은 자리에 적으면 된다. 이 앱이 미사용 출처·`envNote` 에서 반복해 온 원칙과 같다: **모델이 본 것과 사용자가 본 것이 다르면 말한다.**
