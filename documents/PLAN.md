...
- ~~**축소 후 재시도(shrink-and-retry)는 만들지 않았다.**~~ **해소 2026-09-02(9단계)** — 계획대로 **이진 축소**다(`AnswerService.withShrinkRetry()`, 초과 1회당 문서 수 절반, 상한 3단계 → topK=10 이면 10→5→3→2). **자리가 여기인 이유**: 라우터는 호출을 불투명한 클로저로 받아 프롬프트 안을 못 보므로 무엇을 버릴지 아는 곳은 프롬프트를 조립하는 `AnswerService` 뿐이다. 라우터의 프로바이더 순회와는 층이 달라 서로 방해하지 않는다 — 저쪽이 "다른 곳에 물어본다"면 이쪽은 "더 작게 물어본다"다. 답변(블로킹·스트리밍·PROGRESSIVE)과 **검증** 양쪽에 걸린다.
