...
> **2026-09-03 추가 완료**: **§10.13 Direct 턴의 이력 확대**(문서 자리가 비면 그만큼 이력에 돌려준다 — DN 답변의 `## 요약` 유도 · 두 이력 경로의 규칙 통일 · Direct 경로의 예산 안전망).
>
...
> **§10.11 · §10.12 · §10.13 구현 순서 — `10.11 → 10.13 → 10.12`** (§10.11·§10.13 완료, 남은 것은 §10.12)
>
...
- **(b) 이전 턴이 Direct 였을 때의 오염.** 독립화의 재료는 이력인데, DN 답변은 **문서에 근거하지 않은 텍스트**다. `DN → RN` 순서에서 모델이 지어낸 용어가 재작성된 검색어에 섞여 들어가면, 검색은 없는 것을 찾고 RN 답변은 나쁜 검색 위에 세워진다 — 게다가 그 답변이 다시 이력이 되어 같은 용어를 굳힌다. 재료를 검색을 거친 턴으로 제한하거나, 최소한 진단 패널에서 재작성의 출처 턴을 함께 보여줄 것.

  **§10.13 이 먼저 나갔으므로 이 위험은 이미 커져 있다** — 그쪽이 Direct 턴에서 DN 답변을 캡 없이 전문으로 이력에 넣는다. 다만 §10.13 의 1단계가 **DN 답변에도 `## 요약` 을 요구**하게 되면서, 원래 제안했던 판별법("`## 요약` 이 있는 턴 = 검색을 거친 답변")은 **더 이상 성립하지 않는다**. 재료를 제한하려면 `conversation_turns.direct_mode` 컬럼을 직접 보거나(있다), `HistoryPolicy` 가 이미 쓰는 `response_mode` 와 함께 판단해야 한다.
...
- **1단계(DN 도 `## 요약` 을 낸다)는 강제하지 않는다** — 같은 프롬프트를 meta 답변이 쓰므로 인사말에 헤더가 붙으면 안 되고, 형식 미준수와 짧은 답변이 "뺄 게 없으니 전문"이라는 같은 폴백으로 안전하게 수렴한다.
