...
>
> **2026-09-01 추가 완료**: §6.26 컨텍스트 초과 대응(창 인지 + 입력 예산, 10단계 — 인덱싱 재작성 예산 · 선형 축소 재시도 · 창 재탐지 버튼까지) · §6.24 의 남은 이슈였던 토큰 추정 4배 불일치 해소 · C(응용) 모드 운영자 스위치(`app.llm.creative-mode-enabled`) · `/settings` 허용 범위 툴팁과 인덱싱 3종 범위 조정 · `/admin` 문서 레지스트리 페이지네이션 · §6.15 전역 저장 상한(`app.upload.max-total-size`).
...
**10단계로 나눠 진행했다** — 각 단계가 단독으로 가치가 있고 뒤 단계의 전제가 된다.
...
- **`max-tokens` 는 여전히 핫 편집 대상이 아니다.** 프로바이더 빈 생성 시점에 구워지므로 재기동해야 바뀐다(`SettingsKeys.HOT_EDITABLE` 에 없다). 그런데 이 값에서 파생되는 것이 계속 늘고 있다 — 블로킹 상한 · 이력 예산 · MD 교정 섹션 크기 · 컨텍스트 입력 예산 · 인덱싱 출력 상한. 나중에 이걸 핫으로 열려면 **그 다섯 곳이 모두 매 호출 재조회하는지** 함께 확인해야 한다(`MemoryService` 는 이미 그렇게 바꿔 뒀고, `MarkdownCorrectionService.maxSectionChars` 는 아직 생성자 고정이다 — 8단계에서 그 위에 얹힌 **창 파생 값**은 매 호출 재계산이지만, `max-tokens` 파생 상한 자체는 여전히 생성자에서 굳는다).
