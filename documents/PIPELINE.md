...
```
  ...
  6) Markdown 교정 [LLM]
    ...
    - 섹션 경계 컨텍스트 (읽기 전용, 저장 안 됨): 각 섹션은 다음 섹션의 앞부분(lookahead)과 이전
      섹션의 뒷부분(lookbehind)을 각각 최대 BOUNDARY_CONTEXT_LINES(8)줄까지 함께 받는다 — 공백/빈
      줄은 세지 않고 건너뛰어 실제 내용 줄만 8개를 채운다(leadingNonBlankLines()/
      trailingNonBlankLines()). 섹션 경계에서 코드 블록이 잘렸는지 판단하는 용도로만 쓰이고
      교정 결과에는 포함되지 않는다(첫 섹션은 lookbehind 없음, 마지막 섹션은 lookahead 없음).
      경계 컨텍스트가 하나라도 있으면 LLM은 실제 교정 결과를 `<<<RESULT_START>>>`/
      `<<<RESULT_END>>>` 마커로 감싸 반환하도록 지시받고, 코드는 그 사이 텍스트만 결정론적으로
      추출한다 — 마커 밖에 남은 내용(미리보기 잔여물, 서두 등)은 위치·분량과 무관하게 전부
      버려진다. 두 마커 중 하나라도 응답에 없으면 그 섹션은 경계 컨텍스트 없이 재교정한다.
...
| **PPTX** | `PptxToMarkdownConverter` 로 MD 변환 (슬라이드별 `[페이지: N]` + 제목 헤딩 `##` + `[이미지: ...]` 인라인, 본문 불릿은 중첩 목록만) | `MarkdownCorrectionService.correct()` — 섹션 병렬 **포맷 교정** (DOCX·TXT 와 동일 파이프라인이되, 섹션 분할만 `splitByPages()`로 `[페이지: N]` 단위 — §6.3-bis 2번) | `{docId}.md`(원본) + `{docId}_corrected.md`(교정) | **헤딩(슬라이드) 섹션 우선 유지**, 초과 시 섹션 내부 슬라이딩 윈도우 — 단, 서로 다른 슬라이드끼리는 병합되지 않음(§6.3-bis) | `PptxImageExtractor`를 변환기가 직접 호출해 추출 + 본문에 `[이미지: ...]` 인라인(DOCX와 동일) | **지원** |
