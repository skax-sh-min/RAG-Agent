...
> **C는 아직 화면에 없다 (다크 런치).** 서버 구현은 끝났지만 입력창 토글은 여전히 S/N 둘뿐이고, `responseMode=C`는 API로 직접 보낼 때만 닿는다. 오염 방지 게이트(PLAN §6.24 Phase 3)가 끝나기 전에 버튼을 내면, C 답변에 좋아요가 한 번만 눌려도 모델이 지어낸 코드가 `curated_qa`를 거쳐 다음 턴의 "문서"가 된다. 그 게이트 3개(큐레이션 제외·재사용 제외·펜스 절단 복구)는 **모두 닫혔고**, 남은 것은 노출 자체(Phase 4)다.
