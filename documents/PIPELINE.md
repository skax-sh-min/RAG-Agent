...
파일 수신
  ...
  │    PPTX/PDF(비스캔) → 슬라이드/페이지 섹션 단위 유지(mergeShortSections), 초과 시 섹션 내부
  │      슬라이딩 윈도우. 단, 서로 다른 슬라이드/페이지(page_or_slide) 간 병합은 금지 —
...
5. **청킹**: PPTX/PDF(비스캔)는 슬라이드/페이지 섹션 병합(`mergeShortSections`) 전략을 타며, 서로 다른 `page_or_slide`를 가진 인접 섹션끼리는 병합을 금지한다(`ChunkSplitter.isMergeForbiddenByPageMismatch()`) — "청크 1개 = 슬라이드/페이지 1개 = 정확한 인용" 보장을 유지한다. **DOCX/TXT/MD는 이와 별개로 챕터 기반 병합(`mergeSectionsByChapter`, §6.4 표 아래 주석)을 탄다** — `page_or_slide` 값이 없어 이 페이지-경계 금지 규칙은 어차피 no-op이고, 대신 minChunkSize·부모 헤딩 기준의 챕터 병합이 적용된다.
