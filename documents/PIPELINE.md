...
HTTP 요청
  └─ ChatController → AgentService.chat()
        │
        ├─ [병렬] 대화 히스토리 로드
        │         (짧은 후속 질문이면 질의 독립화 LLM ③′) → 질문 유형 분류 (LLM ①)
        │
        └─ AgentGraph.run()
              │
...
| **RETRIEVAL** | 쿼리 확장(조건부 — 15자 미만 질의는 생략, `app.search-multiquery-min-length`; **그 구간은 대신 §10.12 독립화가 담당**해 검색 축 셋이 다시 쓴 질의를 본다) → 확장 LLM 호출과 원본 질의 벡터 검색을 가상 스레드로 병렬 실행(§10.8.1, 원본 검색 지연이 확장 대기 뒤에 숨음) → 배치 임베딩(쿼리 임베딩 캐시 히트 시 스킵) → 벡터 스토어 배치 쿼리(chroma 단일 호출, 결과에 쓰지 않는 임베딩 필드는 요청 자체를 생략 §10.9.1 / sqlite-vec 쿼리별) + 큐레이션 Q&A 축 병렬 조회(§10.10, 예약 version `"curated"`, `search.curated-qa-enabled`로 게이팅 — **벡터와 BM25 를 둘 다 그 네임스페이스로 물어 축 안에서 합친다**. 일반 키워드 축은 문서 version 으로 거르므로 큐레이션 FTS 행은 그쪽 질의로는 나오지 않는다) → 가중 RRF 병합(벡터축 그룹 정규화 + 키워드축 가중치 + 큐레이션축 가중치) → 선택적 LLM 리랭킹(opt-in). 재검색 시 후보 풀 ×1.5씩 **및 최종 컷 +1**(컨텍스트 여유가 있을 때만) 에스컬레이션 + **근거 미사용 하위 청크 교체**(§5) | ③ 쿼리 확장(조건부), [리랭킹 활성 시 1콜] |
| **ANSWER** | 문서 기반 답변 생성 + **충분도·근거 통합 평가**(1콜, §5.1). **검색이 0건이면 두 호출 모두 생략**하고 정형 응답(`chat.answer.no-documents`)을 그대로 답변으로 쓴다 — 모드가 `S` 로 남아 CRITIC 도 건너뛰고, 턴은 `RS` + 미검증으로 저장된다 | ④ 답변, ⑤ 평가 (검색 0건이면 **0콜**) |
...
### 3.1 응답 모드 (S / N / C) — 라우팅 모드와 직교하는 축

사용자가 메시지마다 고르는 **답변의 성격**이다(입력창의 S/N/C 토글, 기본 N — `C`는 `app.llm.creative-mode-enabled`가 켜져 있을 때만 렌더된다). 라우팅 모드(COST_FIRST/PROGRESSIVE)가 "어느 프로바이더로 보낼까"라면, 응답 모드는 "어떤 답변을 쓸까"다.

| 모드 | 성격 | 시스템 프롬프트 | 분량 지침 | 검증 | 좋아요(지식 제안) |
|---|---|---|---|---|---|
| **S** 요약 | 요약 한 섹션 | `prompt.answer.system.s` | **1,000자 이내** + 4~7줄 | 생략(eval + CRITIC 둘 다) | **제외**(좋아요 비활성) |
| **N** 표준 | 문서 충실, 5섹션 | `prompt.answer.system.n` | 숫자 없음 — "구체적이고 자세하게" | `prompt.answer.eval` | 제안 가능 |
| **C** 응용 | 문서를 **재료로** 생성, 4섹션 | `prompt.answer.system.c` | 숫자 없음 — "구체적이고 자세하게" | `prompt.answer.eval.creative` | 제안 가능 |
...
그래서 `ResponseMode.S.allowsSubmission() = false`이고, **S 턴에서는 좋아요 버튼이 비활성으로 렌더된다**(사유는 툴팁에). 사유가 §10.11 에서 바뀌었다 — 예전 근거("구조 섹션을 걷어내면 남는 것이 없다")는 코퍼스에 들어가는 텍스트가 사람이 편집한 것이 되면서 성립하지 않게 됐고, 실제 사유는 **S 가 의도적으로 축약된 답변**이라는 것이다: 배경·이유·전제·예외를 빼라고 지시받은 형식은 오래 남길 지식의 원본이 아니다. **싫어요는 모드와 무관하게 그대로 동작한다**(다음 대화 컨텍스트에서 제외).
...
`ConversationSummarizerService`는 답변의 `## 요약` 섹션을 재요약 없이 그대로 재사용하는데, **S 답변은 전체가 요약**이라 압축 없이 통째로 실린다. RAG 턴의 히스토리 예산은 `max(1000, max-tokens/2)`이므로 `max-tokens=12,000`이면 6,000자 — S 답변 1,000자 기준 약 6턴 분량이다. S의 분량을 늘리면 이 예산을 그만큼 빨리 소모해 오래된 턴이 먼저 밀려난다.

**Direct 턴은 예산이 다르다**(§10.13). Direct 프롬프트에는 `[검색된 문서]` 블록이 통째로 없으므로, 규칙은 "Direct 라서"가 아니라 **"문서 자리가 비어서"**다 — `이력 상한 = 입력 예산 − 문서가 차지할 자리`(Direct 는 0). 기본 설정에서 그 자리는 `topK 10 × chunk-size 1,500` ≈ 15,000자인데 예전에는 모드와 무관하게 5,000자 고정이라, 이력만으로 답해야 하는 모드가 이력을 가장 적게 받고 있었다. **확대만 하는 것이 아니다** — 창이 좁으면 5,000자보다 작게 나오고 그게 옳다(Direct 경로에는 예산 가드가 아예 없었다). 창을 모르면 아무것도 하지 않고, 줄였으면 `budgetNote`로 사용자에게 말한다. 규칙과 렌더(각 턴을 `## 요약`만으로 실을지 본문으로 실을지)는 모두 `HistoryPolicy` 한 곳에 있고 **이력 경로 둘이 함께 읽는다** — 한쪽만 고치면 같은 스레드가 요약 캐시 TTL 을 기점으로 다른 맥락을 본다.
...
| ③ | RETRIEVAL | 쿼리 다양화 (질문 길이 >= `search-multiquery-min-length`) | 없음 |
| ③′ | AgentService (사전) | 질의 독립화 — 짧은 후속 질문을 자립적 검색어로 재작성(§10.12, 길이 < 임계값). ③과 **여집합**이라 둘이 함께 돌지 않는다 | ✓ |
| ④ | ANSWER | 답변 생성 | ✓ |
...
> ③′ **질의 독립화(§10.12, `QuestionCondenser`)** — ③이 생략되는 그 구간(15자 미만 + 이전 질문 있음)에서만 도는 `MICRO_TEXT` 호출이다. 두 게이트가 **여집합**이라 한 턴의 질의 전처리 LLM 호출은 여전히 최대 하나이며(확장 게이트는 재작성 결과가 아니라 **원문 길이**로 잰다), 그래프 바깥(`AgentService`/`StreamingAgentService`의 초기 상태 조립)에서 돌지만 ①과 달리 **토큰이 누적된다**. 결과는 `AgentState.searchQuestion` 에 실려 **벡터·BM25·큐레이션 세 축과 리랭커·분류기**가 함께 쓰고, 답변 프롬프트의 `[현재 질문]` 은 원문 그대로다. 재료는 최근 3턴의 **질문**뿐이며 답변은 넣지 않는다(검색이 자기 답변을 다시 찾는 순환 + Direct 답변의 지어낸 용어 유입 차단). 실패·재료 없음·원문 그대로 반환은 모두 원문 검색으로 수렴한다.  
> ③의 프롬프트는 Spring AI 기본값(영어, 관점 다양화만 요청)이 아니라 `RetrievalService` 생성자가 `messages(_ko).properties`의 `prompt.retrieval.expansion`으로 교체한 커스텀 한국어 프롬프트 — 관점만 다양화하는 게 아니라 인사말·존댓말 어미·군더더기 제거, 지시어/대명사를 구체적 용어로 풀어쓰기 등 **임베딩 벡터 검색에 더 적합한 형태로 정규화**하도록 함께 요청한다. 여기에 **표기 변형**도 포함된다 — 문서가 한글로 쓰였는지 영문으로 쓰였는지 알 수 없으므로 변형 중 최소 1개는 핵심 용어를 반대 언어 표기로 바꾸게 하고("디비 접속 설정" → "database connection configuration"), 약어·구어체 음차(디비)는 정식 표기(데이터베이스/DB)로도 풀어 쓰게 한다. **이 변형은 벡터 축에만 투입된다** — BM25 축(`keywordF`)은 여전히 (③′의 독립화까지 반영한) 질의 하나만 검색하므로, 한/영 표기 차이의 나머지 절반은 키워드 축 가중치(`SEARCH_RRF_KEYWORD_WEIGHT`, 기본 0.5)로 다룬다(OPERATOR_MANUAL §7.8). LLM에 넘기는 질문 텍스트는 다른 프롬프트 구성 지점과 동일하게 `PromptInjectionGuard.wrap()`으로 감싸며, 벡터 검색 자체(원본 질의 축)는 감싸지 않은 원문을 그대로 임베딩한다.
>
...
- **그래도 넘치면 문서를 조금씩 덜어내며 다시 시도한다**(§6.26-9). 위 예산은 전부 `TokenEstimator` 의 **추정** 위에 서 있고, 창을 모르는 배포에서는 아예 돌지 않는다 — 그 두 경우에 초과가 나면 예전에는 사용자가 답변 대신 `RAG-LLM-003` 을 받았다. 이제 문서를 `app.llm.shrink-step` 개(기본 **1**)씩 덜어내며 최대 5회까지 다시 시도한다(기본 조합에서 topK=10 → 10→9→8→7→6→5). **반씩이 아니라 한두 개씩인 이유**: 사전 예산이 이미 창에 맞춰 놓은 뒤라 여기까지 오는 초과는 대개 아슬아슬해서, 반을 자르면 한 개면 됐을 자리에서 근거의 절반이 사라진다. 실패한 시도는 생성 전에 거절되므로 왕복이 싸다. **줄인 사실은 안내 문구에 반영되고**, 검증이 그렇게 줄어든 발췌로 낸 `grounded=false` 는 판정으로 삼지 않는다(축소 판정 가드).
...
**핫 수정 가능** (§6.26 A6, 범위 1,000~32,000): 위 표의 세 소비처가 모두 매 호출 값을 다시 읽고, 프로바이더별 상한도 `MaxTokensCappingChatModel`이 호출마다 다시 계산한다(창 재탐지 §6.26 A5의 결과도 같은 경로로 들어온다). 재기동이 바꾸는 것은 프로바이더 빈의 `defaultOptions` 하나뿐이며, 그건 옵션을 안 싣는 프레임워크 내부 호출자(MultiQuery 확장기)의 폴백이다 — `temperature`와 같은 예외다.
...
발췌 총량 상한은 **32,000자**이며, 초과 시 문서를 **통째로** 하위 순위부터 제외합니다 — 중간을 자르면 검증 대상인 바로 그 값이 사라지기 때문입니다. 기본 설정(10 × 1500 = 15,000자)은 여유롭게 통과하므로 실제로는 과대 설정용 안전판이고, **32,000자에 실제로 닿으려면 64k급 컨텍스트가 필요**합니다(검증 호출은 여기에 답변 전문과 스키마까지 함께 싣습니다). 32k 이하 배포에서는 이 상수보다 `SEARCH_TOP_K × CHUNK_SIZE`가 먼저 한계를 정하므로, 조절 대상은 언제나 그쪽입니다 — OPERATOR_MANUAL §8의 산정표 참고. 제외가 발생하면 `[EVAL] 발췌 한도(글자 … / 토큰 …)로 N개 중 M개만 검증에 사용 — 하위 순위 문서 제외` 경고가 남습니다(§4.0 의 창 기반 토큰 예산도 같은 자리에서 같은 방식으로 걸립니다).
...
파일 수신
  ...
  │    PDF   → 스캔 감지(50자 미만인 페이지가 과반(50% 초과)) 시 페이지 단위 + OCR 자동 적용 (MD 변환 없음)
  ...
  │               PptxImageExtractor 를 내부에서 호출해 data/images/{imageId}/ (imageId = 문서
  │               SHA-256 앞 16자, docId 와 별개 — IMAGE_PROCESS.md §3.3) 에 저장하고, MD 변환
...
4) DOCX → Markdown 변환
  DocumentLoaderService.convertDocxToMd()
    └─ DocxToMarkdownConverter.convert()
      ...
      - 내장 이미지 추출: data/images/{imageId}/d{para}_img{n}.{ext}  (imageId = SHA-256 앞 16자)
      - 본문에는 [이미지: images/{imageId}/{file}] 마커 삽입
      - EMF/WMF는 설정 시 PNG 변환, 실패/미설정 시 [이미지(변환불가): ...] 마커
      - 사진과 같은 문단의 레거시 VML 주석 도형(v:rect/v:oval/v:roundrect/v:line)은 사진 위에
        그려 하나의 합성 PNG로 저장 (app.docx-image.merge-annotated-shapes, 기본 **false**(실험적) —
...
|------|-----------|-----------|------------------------------|------|--------|----------------|
| **PDF(스캔)** | `PagePdfDocumentReader` 페이지 단위. 50% 초과 페이지가 50자 미만이면 스캔 판정 → Tesseract(kor+eng) OCR (`source_type=ocr`). **MD 변환 없음** | 없음 | 없음 | 슬라이딩 윈도우(섹션 병합 없음) | `ImageExtractorService`가 내장 이미지 추출 → `data/images/{imageId}/` | 미지원 |
...
```
PDF 페이지 중 50자 미만인 페이지가 과반(50% 초과)  →  스캔 문서로 판정
