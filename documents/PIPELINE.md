...
④(답변)와 ⑤(검증)는 프롬프트를 조립하기 **전에** 프로바이더의 컨텍스트 창에 맞춰 줄인다. `max_tokens` 가 상한이 아니라 **예약**이기 때문이다 — 서버가 `프롬프트 + max_tokens ≤ n_ctx` 를 검사하므로 쓸 수 있는 입력은 답변 자리를 먼저 잡아둔 나머지다.
...
| 축 | 공식 | topK=10 기준 (최초 → 재시도1 → 재시도2) |
|---|---|---|
| 후보 풀 `candidateK` | `min(topK×(retryCount+1), topK×3)` | 10 → 20 → 30 |
| **최종 컷 `effectiveTopK`** | `topK + retryCount` | 10 → **11** → **12** |
