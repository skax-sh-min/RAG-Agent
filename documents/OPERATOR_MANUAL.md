...
| `SEARCH_CURATED_QA_WEIGHT` | `1.5` | 0.5 ~ 5.0 | §10.10 — 큐레이션 Q&A 축 가중치. 키워드축과 동일하게 그룹 정규화 없이 그대로 적용됨(벡터축 그룹은 항상 `1/축개수`) — `1.0`보다 높을수록 검증된 답변이 우선 노출되되, RRF 자체 순위 감쇠 때문에 무조건 1위를 독식하지는 않음 |
...
- **응답 모드가 `L`(원문 최대)이었던 turn은 좋아요를 눌러도 임베딩 자체가 시작되지 않습니다** — L모드 답변은 검색된 원문 표현을 최대한 그대로 보존하므로 이미 인덱싱된 원본 문서 내용과 사실상 동일하며, 다시 임베딩하는 것은 불필요한 API 호출입니다. `curated_qa` 행은 다른 모드와 동일하게 생성되어 좋아요 취소·본인 편집·관리자 목록/강제삭제가 정상 동작하고, `embed_status`도 "실패"가 아닌 기본값(`ok`)으로 남습니다 — 시도 자체를 하지 않았기 때문입니다. 이 판정은 `conversation_turns.response_mode`(turn 생성 시 저장된 값)를 기준으로 합니다. S/M/L 응답 모드가 `LLM_MAX_TOKENS`를 어떻게 활용하는지는 [§3.2 `LLM_MAX_TOKENS`](#32-환경변수-전체-목록)를 참고하세요.
...
- 답변 텍스트 끝에 LLM이 자동으로 붙이는 "## 참고"(출처 인용) 섹션은 저장·표시에는 그대로 남지만, 임베딩(검색 매칭)에는 포함되지 않습니다 — 파일명·페이지 번호가 질문 의도와 무관한 노이즈이기 때문입니다.

**대화 삭제와의 관계**: 대화(thread)를 통째로 삭제해도 이미 승격된 큐레이션 Q&A는 **유지됩니다** — 좋아요로 공유 지식화된 내용을 개인 대화 정리 행위가 조용히 지우면 다른 사용자의 검색 품질이 저하되기 때문입니다. 채팅창에서 대화를 삭제하려 하면, 그 안에 좋아요한 답변이 있을 경우 확인창에 이 사실이 안내됩니다. 완전히 회수하려면 삭제 전에 좋아요를 먼저 취소하거나, 관리자가 아래 §7.5에서 강제 삭제해야 합니다.
...
| `curated_qa` | `memory.db` | 승인된 제안의 실제 색인 항목 — §6.7 좋아요 승격 항목과 같은 테이블(`origin='manual'`로 구분) |
...
**입력 제한** (모두 `CuratedSubmissionService`):

| 항목 | 값 | 비고 |
|---|---|---|
| 제목 최대 길이 | 200자 | `curated_qa.question` 컬럼에 그대로 저장됨 — 임베딩 입력의 앞부분이라 질문형 제목일수록 검색이 잘 됨 |
| 본문 최대 길이 | `app.chunk-size`(=`CHUNK_SIZE`) | **정규화 후** 길이 기준(장식 마크다운 제외). §6.5에서 핫 수정하면 즉시 반영됨 |
| 사용자당 검토 대기 상한 | 20건 | 초과 시 등록 거부 |

> **본문 길이 검증은 장식이 아니라 필수입니다.** §6.7의 "입력이 너무 큼" 재시도는 답변의 `## 상세 설명` 같은 섹션 구조를 잘라내 축소하는 방식인데, 손으로 쓴 제안에는 그 구조가 없어 **폴백이 동작하지 않습니다** — 상한을 넘긴 본문은 그냥 임베딩에 실패합니다. `CHUNK_SIZE`를 크게 올렸다면 임베딩 서버의 배치/토큰 한계(§8 `input (N tokens) is too large`)도 함께 확인하세요.
>
> 한 건은 항상 청크 하나입니다 — 자동 분할은 하지 않습니다(승인/거부/철회가 여러 벡터에 걸치면 상태 관리가 복잡해지므로).
...
>
> **주의 2**: `SEARCH_TOP_K`(기본 8)는 그대로인데 청크가 잘게 쪼개지면 한 번에 들어오는 총 컨텍스트 양이 줄어듭니다. 최대 분할로 바꾼 뒤 답변이 얕아지면 `SEARCH_TOP_K`를 함께 올리세요 — 둘 다 `/settings` 핫 수정 대상입니다.
>
...
- 사용자가 직접 등록한 제안이 승인되면 이 카드에 **함께** 나타납니다(내부적으로 `origin='manual'`) — 편집·강제 삭제 방법은 좋아요 항목과 동일합니다. 여기서 강제 삭제하면 §7.6 목록에서는 "회수됨"으로 표시됩니다.
...
| 기능 | 방법 |
|------|------|
| 내용 확인 | 행의 아이콘 → 오른쪽 오프캔버스에 제목·본문 **전문** 표시(작성자 id 포함) |
...
- 승인/거부는 `status='pending'`을 조건으로 하는 compare-and-set이라, 관리자 두 명이 동시에 눌러도 색인 항목은 하나만 생성됩니다(진 쪽은 409 + "이미 처리된 제안입니다" 안내, 방금 만든 항목은 자동 회수).
