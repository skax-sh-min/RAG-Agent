...
- **검색 가중치는 제안 게시판 항목과 동일**합니다: 두 경로 모두 같은 `"curated"` 네임스페이스에 들어가고 `SEARCH_CURATED_QA_WEIGHT`(기본 1.5)가 축 전체에 적용되며, origin(좋아요/수동)으로 구분하지 않습니다. 다만 **긴 항목일수록 청크가 많아 그 축에서 차지하는 후보 수가 늘어난다**는 점은 알아두세요 — 한 답변이 상위권을 여러 칸 차지할 수 있습니다.
