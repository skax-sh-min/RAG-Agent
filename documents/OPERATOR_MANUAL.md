...
- **그래도 넘치면 절반씩 줄여 다시 시도합니다** — 로그 `[SHRINK] ANSWER 컨텍스트 초과 — 프롬프트를 줄여 다시 시도한다. 단계 0→1, 문서 10→5개`. 최대 3단계(10→5→3→2)이고, 거기서도 안 되면 `RAG-LLM-003` 이 나갑니다. **이 로그가 자주 보이면 예산 계산이 실제와 어긋나 있다는 뜻입니다** — `[TOKEN_CAL]` 계수를 함께 확인하고, `SEARCH_TOP_K` 를 낮추거나 서버 `--ctx-size` 를 키우십시오. 재시도로 답변은 나가지만 그때마다 왕복이 한 번 더 듭니다.
