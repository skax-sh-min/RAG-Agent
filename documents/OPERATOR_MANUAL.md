...
| `SEARCH_CURATED_QA_WEIGHT` | `1.2` | 0.5 ~ 5.0 | §10.10 — 큐레이션 Q&A 축 가중치. 키워드축과 동일하게 그룹 정규화 없이 그대로 적용됨(벡터축 그룹은 항상 `1/축개수`) — `1.0`보다 높을수록 검증된 답변이 우선 노출되되, RRF 자체 순위 감쇠 때문에 무조건 1위를 독식하지는 않음 |
| `SEARCH_SUBMISSION_WEIGHT` | `1.5` | 0.5 ~ 5.0 | **지식 제안**(승인된 사용자 제출) 축 RRF 가중치 (hot). 좋아요 큐레이션과 같은 벡터 네임스페이스·같은 검색을 쓰되 `curated_origin` 메타데이터로 **서로 다른 RRF 축**으로 갈린다 — 신뢰 근거가 다르므로 따로 조절한다([§6.7](#67-큐레이션-qa-좋아요-기반-지식-승격-1010)) |
...
- `ui.retrieval-metrics-enabled`를 켜면 출처마다 `유사도 0.72 · 검색 18% · vec:2, bm25:5 · 응답 31%`가 함께 표시됩니다 — 검색 튜닝(§6.5의 RRF 가중치·유사도 임계값·하이브리드 검색)의 효과를 실제 질의에서 눈으로 확인하기 위한 값입니다. 계산은 RRF 융합의 부산물이라 **LLM·임베딩 추가 호출이 없고**, 끄고 켜는 것으로 검색 결과가 달라지지 않습니다(표시만 제어).
  - **읽는 법**: `유사도`는 질의와의 코사인 거리라 변별력이 크고, `검색 %`는 순위 기반(RRF)이라 의도적으로 평평합니다 — 1위와 8위가 크게 차이 나지 않는 것이 정상입니다. 진단에 가장 유용한 건 축별 순위(`vec:12, bm25:1` = "벡터로는 안 잡혔는데 키워드로 올라옴")입니다. `검색 %`의 정확한 정의와 "값이 벌어지면 무슨 뜻인지"는 [§7.7 검색기여 읽는 법](#77-검색-진단-수치--검색-튜닝-근거-보기) 참고.
