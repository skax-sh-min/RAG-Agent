...
| 오류 유형 | 차단 시간 | 비고 |
|----------|----------|------|
| HTTP 429 (Rate Limit) | `Retry-After` 헤더 값 | 헤더 없으면 §6.12 개선안 4(아래)에 따라 폴백 가능 여부로 분기 |
| HTTP 402 (결제 필요) | `Retry-After` 헤더 값 | 헤더 없으면 위와 동일 |
| HTTP 503 (Service Unavailable) | `Retry-After` 헤더 값 | 헤더 없으면 위와 동일 — §6.12 이전에는 "기타 오류"로 분류돼 항상 30초였음 |
| 그 외 4xx/5xx, 네트워크 오류 | 30초 고정 | 변경 없음 |

**§6.12 개선안 4 — 폴백 없는 프로바이더의 과부하성 오류(429/402/503) 완화**: `Retry-After` 헤더가 없을 때, 이 요청에서 시도 가능한 다른 프로바이더가 남아있으면(폴백 존재) 기존과 동일하게 `circuit-breaker-minutes` 전체를 차단합니다 — 다음 프로바이더로 정상 전환되므로 문제가 없습니다. 반면 **폴백이 전혀 없는 유일한 프로바이더**(전형적으로 단일 LOCAL 프로바이더만 등록된 배포)라면, 전체 시간 차단은 다음 요청부터 만료 시각까지 서비스 전체를 다운시킬 뿐이므로 **30초로 단축 차단**합니다 — §6.12 동시성 게이트(§5.7)가 이미 프로바이더에 걸리는 부하를 억제하고 있어 짧게 재시도해도 안전합니다. 서버가 명시적으로 `Retry-After`를 보낸 경우는 폴백 유무와 무관하게 항상 그대로 존중됩니다.
