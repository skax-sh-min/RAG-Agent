...
- **↺ 버튼 (MD 재인덱싱)**: DOCX·TXT·PPTX·PDF(스캔이 아닌 경우) 문서는 서버에 저장된 교정 MD 파일(`{docId}_corrected.md`)을 기준으로 청크를 다시 생성하고 인덱싱합니다. MD 파일을 직접 수정한 뒤 이 버튼을 누르면 재업로드 없이 인덱싱을 갱신할 수 있습니다. 스캔 PDF는 MD 파일 자체가 없어 지원되지 않습니다. MD 파일이 가리키는 이미지가 실제로는 존재하지 않으면(수동 정리 등으로 파일이 사라진 경우) 해당 이미지 참조는 재인덱싱 시 자동으로 제거되고 MD 파일에도 반영됩니다.
