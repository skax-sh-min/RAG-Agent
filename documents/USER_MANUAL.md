...
`/admin` 접속 후 ChromaDB에 저장된 청크를 직접 조회·편집·삭제할 수 있습니다.

> **벡터 스토어 백엔드 주의**: 이 청크 브라우징/편집 기능은 **ChromaDB 백엔드 전용**입니다. `VECTORSTORE_TYPE=sqlite-vec` 모드에서는 컬렉션 목록이 비어 있게 표시되며(편집 미지원), 재인덱싱은 문서 재업로드로 수행합니다.

**왼쪽 패널 — 컬렉션(버전) 목록**:
- ChromaDB에 존재하는 컬렉션과 버전별 청크 수 표시
- 클릭 시 오른쪽 패널에 해당 컬렉션의 청크 목록 로드
