...
표에는 ID·텍스트 미리보기·크기·파일명·페이지/슬라이드·**챕터**·키워드가 함께 표시됩니다. 챕터 열은 §4.2의 `chapter_no`(H2~H6 헤딩 기반 계층 번호)를 보여주며, 실제 챕터가 없는 청크(프롤로그·PPTX·PDF)는 빈 칸으로 표시됩니다.
...
`/admin` 페이지 하단 **큐레이션 Q&A** 카드에서 사용자들이 좋아요한 전체 답변을 최신순으로 확인할 수 있습니다.
