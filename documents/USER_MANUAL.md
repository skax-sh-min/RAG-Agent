...
| 업로드/동기화 취소 | 문서 업로드·폴더 동기화 진행 중 언제든 취소 가능 (파일별 · 동기화 전체) |
...
| 관리자 패널 | ChromaDB 컬렉션·청크 조회·편집·삭제 + 저장된 MD 파일 기반 재인덱싱 UI (`/admin`) |
...
| **업로드/동기화 취소** | 진행 중인 파일 행의 **Cancel** 버튼(업로드/인덱싱 중), 또는 동기화 진행바 옆 **Cancel** 버튼 |
| **목록 새로고침** | **Refresh** 버튼 |
...
- 대용량 파일이나 폴더 동기화 도중 잘못 시작했다면 **Cancel**로 즉시 중단할 수 있습니다. 취소 시점까지 이미 완료된 파일은 그대로 인덱싱되어 남고, 진행 중이던 파일만 취소됩니다
...
`/admin` 접속 후 벡터 스토어에 저장된 청크를 직접 조회·편집·삭제할 수 있습니다. 상단에는 **Vector Store 상태 카드**(백엔드 종류·정상 여부·문서/청크 수·버전별 청크 수)가 표시됩니다.
