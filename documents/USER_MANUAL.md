...
- **SSE 실시간 스트리밍** — 노드 진행 단계 배지 + 토큰 실시간 표시; DUAL 모드는 탭별 스트리밍
...
- **이미지 처리 파이프라인** — PDF·PPTX·DOCX 이미지 추출 → 검색 결과에 근거 이미지 썸네일 표시; 스캔 PDF OCR 지원
...
5. 답변이 SSE 스트리밍으로 실시간 표시됨:
   - 상단 배지에서 현재 처리 단계 확인 (질문 분류 → 문서 검색 → 답변 생성 → 검증)
   - 토큰이 생성되는 즉시 화면에 출력
6. 답변 버블에서 출처(Sources)·소요 시간·토큰 수 확인 가능
   - 출처 항목에 마우스를 올리면 관련 문서 청크 텍스트 미리보기 팝오버 표시
   - 검색된 이미지가 있으면 답변 아래 썸네일 그리드로 표시 (클릭 시 원본 확인)
...
- PDF·PPTX·DOCX 파일은 이미지를 자동 추출하여 검색 결과에 포함 (설정 필요: `app.image-description.enabled=true`)
- 스캔 PDF는 Tesseract OCR로 텍스트 추출 (`app.image-description.ocr-enabled=true`)
...
|------|----------|----------|------------|
| `.md` | `#` / `##` / `###` 헤더 단위 섹션 | 섹션이 `CHUNK_SIZE` 초과 시만 슬라이딩 윈도우 적용 | URL 이미지 → alt 텍스트 유지, 로컬 이미지 → `[이미지: path]` 마커 |
| `.docx` | Word `Heading1` / `Heading2` 스타일 단위 섹션; 이미지를 Markdown `[이미지: ...]` 마커로 변환 | 섹션이 `CHUNK_SIZE` 초과 시만 슬라이딩 윈도우 적용 | `DocxToMarkdownConverter`로 인라인 이미지 추출 + EMF/WMF → PNG 변환 (설정 시) |
| `.pptx` | 슬라이드 1장 = 청크 1개 | 추가 분할 없음 | `PptxImageExtractor`로 슬라이드 이미지 추출 |
| `.pdf` | 페이지 1장 = 문서 1개 | 슬라이딩 윈도우 (`CHUNK_SIZE` / `CHUNK_OVERLAP`) | `PdfImageExtractor`로 임베드 이미지 추출; 스캔 PDF는 OCR 처리 (설정 시) |
