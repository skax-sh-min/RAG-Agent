...
| `ingestion/ChunkSplitter.java` | Pure chunk-splitting/merging algorithm (section-aware merge for MD/DOCX/TXT, sliding window otherwise); no Spring bean dependencies, extracted from `DocumentIndexer`; size-budget checks in `mergeShortSections()`/`enforceMaxChars()` measure normalized length (`MarkdownNoiseNormalizer`) — `slidingWindow()` and its offset-based helpers stay on raw text |
