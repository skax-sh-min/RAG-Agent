...
| `ingestion/KeywordSearchRepository.java` | SQLite FTS5 `chunk_fts` index (BM25 keyword axis for hybrid search) + 동반 일반 테이블 `chunk_fts_key`(`spring_doc_id → rowid`, 위치 컬럼, 본문 해시). **id 로 FTS 행을 찾는 조회는 전부 `chunk_fts_key` 를 먼저 타고 `rowid` 로 조인한다** — `chunk_fts` 를 `spring_doc_id`/`doc_id` 로 직접 거는 WHERE 는 코퍼스 전체 스캔이다 [↗](documents/PITFALLS.md#ingestionkeywordsearchrepositoryjava) |
