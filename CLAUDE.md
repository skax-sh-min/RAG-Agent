...
- §6.13 settings override layer: `/settings` shows the effective LLM/RAG config; a small set of search-tuning values are **hot-editable** (apply on the next search, no restart). Mechanism — `AppProperties` holds a process-wide **static** `OverrideSource` (it's an immutable `@ConfigurationProperties` record, so it can't take the store as a ctor arg nor an injected field); `SettingsService` binds itself via `AppProperties.bindOverrides(this)` at `@PostConstruct` and unbinds at `@PreDestroy`. The 7 hot accessors (`searchSimilarityThresholdSafe`, `searchRrfKeywordWeightSafe`, `searchRrfKSafe`, `searchCandidateMultiplierSafe`, `searchTagCandidateMultiplierSafe`, `searchMultiqueryMinLengthSafe`, and the new `searchRetryEscalateSafe`) read **override → else property default** then apply the existing clamping; when unbound (unit tests) they behave exactly as before (regression 0). Keys are in `SettingsKeys`; overrides persist in `settings_override` (memory.db, `SettingsOverrideRepository`) and revert to the property default on delete. **`RetrievalService` must read these values fresh via `props.xxxSafe()` on each `execute()`/`shouldExpand()`** — it used to cache them in the constructor, which would defeat hot-reload; structural values (topK, multiquery/hybrid/rerank-enabled) stay cached (restart-required). Writes go through `/admin/settings/**` (inherits §6.17 `/admin/**` ROLE_ADMIN gating — no new SecurityConfig/`NoAuthAutoLoginFilter` wiring) and are audited (`settings.update`/`settings.reset`). temperature/max-tokens and default routing mode are view-only (temperature needs §6.18; routing mode has no runtime consumer). Adding a new hot key = add to `SettingsKeys` + `SettingsService.HOT_SPECS` + the override branch in the matching `xxxSafe()`; every `xxxSafe()` must map 1:1 to a record component or `AppPropertiesSafeAccessorTest` fails
