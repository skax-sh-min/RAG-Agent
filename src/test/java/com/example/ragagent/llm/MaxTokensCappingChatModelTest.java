...
class MaxTokensCappingChatModelTest {
    ...
    @Test
    @DisplayName("상한은 호출마다 다시 계산된다 — /settings 의 max-tokens 편집과 창 재탐지가 이 경로로 들어온다")
    void theCeilingIsRecomputedPerCall() {
        var seen = new AtomicReference<Prompt>();
        var live = new java.util.concurrent.atomic.AtomicInteger(4_000);
        var capping = new MaxTokensCappingChatModel(delegateCapturing(seen), "small", live::get);

        assertThat(maxTokensSeenBy(capping, seen, promptWith(10_000))).isEqualTo(4_000);

        live.set(8_000);   // 운영자가 /settings 에서 올렸거나, 재탐지로 창이 넓어졌다
        assertThat(maxTokensSeenBy(capping, seen, promptWith(10_000)))
                .as("생성자에서 값을 받아 두면 재기동 전까지 4,000 에 묶인다")
                .isEqualTo(8_000);
    }
}
