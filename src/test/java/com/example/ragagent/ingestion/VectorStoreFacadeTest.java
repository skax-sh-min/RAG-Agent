...
class VectorStoreFacadeTest {
    ...
    private VectorStoreFacade facade(VectorStoreRegistry registry, ChromaApi chromaApi,
                                     EmbeddingModel embeddingModel, double threshold) {
        AppProperties props = mock(AppProperties.class);
        when(props.searchSimilarityThresholdSafe()).thenReturn(threshold);
        return new VectorStoreFacade(registry, chromaApi, embeddingModel, new ObjectMapper(), props);
    }
    ...
    private SearchRequest capturedSearch(double threshold) {
        ...
        facade(registry, mock(ChromaApi.class), mock(EmbeddingModel.class), threshold)
    ...
    private VectorStoreFacade batchFacade(EmbeddingModel embeddingModel, ChromaApi chromaApi, double threshold) {
        VectorStoreRegistry registry = mock(VectorStoreRegistry.class);
        ...
        return facade(registry, chromaApi, embeddingModel, threshold);
    }
    ...
    void searchBatch_groupsPerQuery_andScores() {
        ...
        List<List<Document>> result = batchFacade(embeddingModel, chromaApi, 0.0)
    ...
    void searchBatch_appliesThreshold() {
        ...
        List<List<Document>> result = batchFacade(embeddingModel, chromaApi, 0.5)
                .searchBatch("owner", List.of("q1", "q2"), "latest", 7);
    ...
    void searchBatch_noCollection_returnsEmpty() {
        ...
        List<List<Document>> result = facade(registry, chromaApi, embeddingModel, 0.0)
                .searchBatch("owner", List.of("q1", "q2"), "latest", 7);
