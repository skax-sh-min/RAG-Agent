...
class ChatControllerHtmxTest {
     ...
     * 저장해서 — 5섹션 N 답변을 S 로 골라 둔 채 재사용하면 새로고침 뒤 [RS] 로 찍혔고, 그 턴의
     * 좋아요→지식 제안 프리필은 태그 스코프를 잃었다. 응답에도 같은 값이 실려야 화면이 폼 값 대신
     * 그것으로 그린다(새로고침 전후 표기 일치). 컨트롤러는 서비스가 준 값을 그대로 복사할 뿐이라
     * directMode=true 도 전달 여부만 본다 — 실제로는 리포지토리가 Direct 턴을 후보로 내지 않는다.
     */
    @Test
