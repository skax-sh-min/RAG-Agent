...
public enum ResponseMode {
     ...
     * 뿐이라, 이미 그 모드로 답한 과거 턴의 두 글자 표기({@code [RC]})·배지는 계속 그려진다.
     *
     ...
     *
     * <p>이 후처리는 PLAN §6.24 Step 1-c에서 <b>조건부 가드</b>로 강등됐다({@code SummaryOnlyGuard}) —
     * 전용 시스템 프롬프트가 생겨 모델이 애초에 요약만 쓰므로, 후처리는 그게 실패했을 때만 도는
     * 안전망이자 프롬프트 효과의 측정 수단이다.
     */
