...
| `AUTH_ENABLED` | — | `true` | Spring Security form login on/off (`app.auth.enabled`). `false` = no-login deployment: CSRF off, stateless sessions, `NoAuthAutoLoginFilter` injects a guest/admin identity and redirects to `/setup` until an admin exists — see [OPERATOR_MANUAL.md §9.4](documents/OPERATOR_MANUAL.md) |
| `AUTH_MANAGEMENT_ONLY` | — | `false` | Only meaningful with `AUTH_ENABLED=false` (`app.auth.management-only`). Chat and browsing stay guest-open while `/admin/**` and the document-write routes require a real login (gated `hasRole("ADMIN")`, not merely authenticated) |
| `AUTH_GUEST_IDENTITY` | — | `shared` | How a no-auth visitor gets a `userId` (`app.auth.guest-identity`): `shared` (one guest for everyone, the pre-existing behavior) / `ip` / `cookie` / `hybrid` (recommended — long-lived `rag_visitor` cookie, IP otherwise). An unknown or blank value normalizes back to `shared` |
...
| `LOCAL_LLM_MODEL` | — | `google/gemma-4-e4b` | `providers[1]` model name — the id from the server's `/v1/models`, **or any part of it that no other model there shares** (e.g. `gemma-4-e4b` when LM Studio reports `google/gemma-4-e4b`; case-insensitive). G3 resolves it to the server's full id at startup and that is what every request carries |
| `LOCAL_LLM_TYPE` | — | `BOTH` | `providers[1]` task type (`app.llm.providers[1].type`): `MICRO_TEXT`/`LIGHT_TEXT`/`TEXT`/`VISION`/`LIGHT_BOTH`/`BOTH`. `BOTH` handles everything. The three text types form a ladder (`MICRO_TEXT` ⊂ `LIGHT_TEXT` ⊂ `TEXT`), so `TEXT` still takes every text chore (keywords, summaries, MD correction…) — it only drops Vision, which is a separate axis. Prefer leaving a text-only model at `BOTH`: one failed Vision call is remembered and only image work is skipped afterwards |
...
| `LLM_VERIFY_LOCAL_MODELS_ON_STARTUP` | — | `true` | (`app.llm.verify-local-models-on-startup`) — the G3 toggle. When `true`, every registered LOCAL-role provider (any of the three above with a URL set) is checked at startup via `GET {url}/models`; a name matching none (or several) of the server's models, or a connection failure, aborts startup entirely. Set `false` only if your local server reliably starts *after* this app in your deployment order — with it off, an unreachable/mismatched local provider degrades to the old runtime fallback (one failed call, then routed elsewhere) instead of blocking startup |
...
| `LLM_MAX_TOKENS` | — | `10000` | Completion-length cap for **blocking** LLM calls only (classification, keyword extraction, MD correction, TXT structuring, etc.) — streaming chat/Direct answers are uncapped by design (bounded by SSE timeouts instead). Also sizes the conversation-history budget and MD-correction section-splitting budget (same value, shared across all three). **The evaluation call is exempt**: it caps its own reply at 2,048 tokens, since a few JSON fields must not reserve the whole completion budget inside a narrow context window. **Not the model's context window** — size it with headroom under your LLM server's actual context size (`--ctx-size`); the `10000` default assumes a **32k-or-larger** window, so lower it to ~2000 on an 8k-10k model. See [PIPELINE.md §4.1](documents/PIPELINE.md#41-appllmmax-tokensllm_max_tokens-크기-산정--로컬-llm-컨텍스트-윈도우와의-관계) and [OPERATOR_MANUAL §8](documents/OPERATOR_MANUAL.md#8-문제-해결) Hot-editable from `/settings` (1000-32000). |
...
| `LLM_INDEXING_TEMPERATURE` | — | `0.0` | Temperature for every ungated background/indexing call (`app.llm.indexing-temperature`) — keyword extraction, MD correction, TXT structuring, vision image description/classification, thread-title generation, conversation summarization. Separate from `LLM_TEMPERATURE`/`DIRECT_LLM_TEMPERATURE` so these extraction-style calls stay deterministic regardless of what those are set to. Clamped to `[0.0, 0.1]` — enough to break a local model that repeats itself at exactly 0, and no more: these outputs get parsed (keywords, JSON verdicts, structured MD), so sampling variety is a defect here rather than a feature. **Hot-editable via `/settings`** — applies to the next call without a restart |
| `CREATIVE_LLM_TEMPERATURE` | — | `0.7` | Temperature for the **C (응용)** response mode only (`app.llm.creative-temperature`), clamped to `[0.0, 1.0]`. A separate knob because `LLM_TEMPERATURE` is clamped to `[0.0, 0.3]` — a document-faithful answer must not wobble under sampling, which also makes creative generation impossible on that setting. **Hot-editable via `/settings`**, and applied on **both** the blocking and the streaming path |
| `LLM_SHRINK_STEP` | — | `1` | Documents dropped per retry when the LLM server rejects the prompt for exceeding its context window (`app.llm.shrink-step`). The pre-call budget already fits the prompt to the window, so an overflow that gets this far is usually marginal — hence a linear step rather than halving. Retries are capped at 5, so this value x 5 is the largest reduction reachable before `RAG-LLM-003`. Hot-editable. |
| `CREATIVE_MODE_ENABLED` | — | `true` | Whether the **C (응용)** response mode can be picked at all (`app.llm.creative-mode-enabled`). The temperature above says *how* C answers; this says *whether* C is open — it is the only mode that generates content from outside the retrieved documents, so some deployments should simply not offer it. `false` removes the C button from the chat input entirely (not merely disabled) and downgrades to `N` any request that still asks for C — REST calls and hand-made form posts included; turns already answered in C keep their `[C]` label and badge. **Hot-editable via `/settings`** |
...
| `GEMINI_MODEL` | — | `gemini-3.5-flash-lite` | Overrides the model for both Gemini NORMAL-tier providers (`providers[3]` gemini-flash-lite, `providers[4]` gemini-flash). They are a load-balancing pair — same model by default, same priority, one API key each — so this one var moves both; to run two different models, set each `providers[N].model` in `application.properties` instead |
...
| `UPLOAD_MAX_TOTAL_SIZE` | — | `0` (unlimited) | Deployment-wide storage cap; accepts `20GB`/`500MB`, a bare number is bytes. Measures `documents` + `converted` + `images` under `DATA_DIR` — indexing's converted Markdown and extracted images, not just the uploaded originals. Over it, an upload is refused with 413 `RAG-UP-002`. Excludes `documents/backup/` (see below), so deleting a document really does free space — see [OPERATOR_MANUAL.md](documents/OPERATOR_MANUAL.md#업로드-크기-제한) |
| `BACKUP_RETENTION_DAYS` | — | `30` | Retention for `documents/backup/`, where deleting a document archives its original. `0` = no age limit |
| `BACKUP_MAX_SIZE` | — | `2GB` | Size cap for the same directory — oldest archives go first. `0` = no size limit. Only this app's own archives are ever deleted |
...
| `RATE_LIMIT_UPLOAD_PER_MINUTE` | `10` | Requests that **accept a new file** per minute (`POST /ui/documents/upload`, `POST /api/v1/documents`, and the guest-open proposal image upload `POST /curated/submissions/images`). Viewing the documents page, refreshing its list, exports and tag edits use the default bucket |
...
| `SEARCH_TOP_K` | `10` | 2 ~ 15 | Number of documents returned by vector search. Also sets how much evidence rides in the verification call (`topK × CHUNK_SIZE`, this app's largest single request) — lower it first when the model's context window is tight |
| `SEARCH_SIMILARITY_THRESHOLD` | `0.3` | 0.0 ~ 0.75 | Min cosine similarity to keep a chunk (`0.0` = accept all). Applies to the **vector axis only** (inside `VectorStoreProvider`, so the curated axes are pruned too, but BM25 is not) — raising it therefore also raises the keyword axis's relative share of the fused ranking, so don't raise this and `SEARCH_RRF_KEYWORD_WEIGHT` in the same step |
| `SEARCH_MULTIQUERY_ENABLED` | `true` | true/false | Query pre-processing on/off — gates **both** multi-query expansion and the §10.12 follow-up condense below (same kind of call, one switch) |
| `SEARCH_MULTIQUERY_MIN_LENGTH` | `15` | 0 ~ 20 | The boundary between the two query pre-processing calls, not just a skip threshold. **At or above it**: multi-query expansion (the original-question search runs in parallel with it rather than behind it). **Below it**: §10.12 condense — a short follow-up (`"그 설정은 어디야?"`) is rewritten into a standalone search query using the recent *questions* (never the answers), and that one value feeds the vector, BM25 and curated axes plus the reranker and classifier; the answer prompt still gets the original wording. The two are complements, so a turn never spends more than one such call. `0` = expansion always, condense never |
...
| `SEARCH_RETRY_ESCALATE` | `true` | true/false | Escalate on each **re-search** — one flag, two axes: candidate pool `round(topK × (1 + 0.5 × re-searches))` capped `×3`, **and** the final cut `topK + re-searches` (applied only when the verification call has room for it). Growing the pool alone only changes which documents compete for the same topK slots, so evidence sitting just past the cut stays out on every attempt. A retry also **evicts** up to a third of the previous cut — chunks the evaluator reported not using, from the bottom of the ranking — which is why the pool no longer has to double |
...
| `SEARCH_RRF_KEYWORD_WEIGHT` | `0.5` | 0.5 ~ 3.0 | Weighted RRF (Phase 7-A) — BM25 keyword axis weight. Vector axes (1-3 MultiQuery variants) are always group-normalized to `1/axisCount`, so `1.0` would be parity with the normalized vector group; the default deliberately sits at half that. BM25 (`chunk_fts`, `tokenize='trigram'`) is a literal same-script matcher — a Korean question shares no trigram with an English-written chunk, so that chunk gets no keyword support at all, while unrelated chunks repeating a common token still take rank 1-2 on the axis. Raise it toward `1.0` if your corpus is single-language and exact term matching (filenames, error codes, API names) matters more than semantic recall. No effect when `SEARCH_HYBRID_ENABLED=false` |
...
For a **RAG turn** the conversation-history budget is auto-derived as `LLM_MAX_TOKENS × 0.5` (floor 1,000 chars). Both the raw-history fallback path and the precomputed-summary path (below) are handed that same ceiling, so switching between them never changes *how much* context reaches the LLM.

For a **Direct turn** (§10.13) the ceiling comes from the provider's context window instead — `input budget − the space documents would have taken`, which is zero here because no retrieval runs. That can be larger *or smaller* than the RAG value: on a narrow window the honest number is below the old fixed ceiling, and the Direct path had no budget guard before. When the budget forces history out, the answer says so (`budgetNote`). Both paths read the one rule in `HistoryPolicy`, which also decides *what* each past turn is rendered as (summary section vs. full body) — see [PIPELINE.md §3.1](documents/PIPELINE.md).
...
└── src/main/
    ...
    │   │   └── WebConfig.java         # ChatClient bean + i18n (CookieLocaleResolver); deliberately registers no CORS mapping
    │   ├── audit/
  ...
  └─▶ [Classifier]  → Classify question type (concept / usage / error / version / meta)
        ...
        └─ other ──▶ [Retrieval]   (LLM generates optimal query → vector search)
                       └─▶ [Answer]   (Structured answer + sufficient self-evaluation)
                              ├─ Insufficient evidence ──▶ [Retrieval] (re-search, up to 2 retries)
                              └─ Sufficient           ──▶ [Finalize] (response mode S — verification skipped)
                                                     └─▶ [Critic]   (N: grounded · C: apiGrounded)
                                                             ├─ Ungrounded ──▶ [Answer]  (rewrite against the same evidence — no re-search)
...
- **Settings page (`/settings`)** — view the effective LLM/RAG configuration (providers, routing, embedding, search tuning) in one place; several families of values are **hot-editable without a restart** (persisted in `settings_override`, revert to the property default on delete): search tuning (similarity threshold, RRF weight/k, candidate multipliers, multi-query min length/enabled, retry escalation, topK, hybrid search — apply on the next search), indexing/chunking (chunk size/overlap/min, **chunking strategy**, concurrent file/LLM-call limits — apply on the next indexing or ↺ re-index), all four LLM temperatures — general/RAG, Direct-answer, indexing/background, and the C-mode creative temperature (each applies on the next call of its own kind) — and a UI toggle for the chat source-preview popovers (`ui.source-preview-enabled`, applies on the next page render). The blocking-call output cap (`max-tokens`) and the context-overflow shrink step are hot-editable too, and a **컨텍스트 다시 탐지** button re-probes each local provider's context window (§6.26). Editing is admin-only and audited; restart-required values (rerank-enabled, embedding config, vector-store backend, default routing mode, storage cap) are shown read-only
...
- **Context-window awareness** — each provider's window is either declared (`app.llm.providers[N].context-size`) or probed at startup (llama.cpp `/props`, LM Studio `/api/v0/models`); the "model maximum" fields those servers also expose are deliberately ignored, since a server started with `-c 8192` still reports 131072 for them and a budget built on that number causes the very overflow it was meant to prevent. When the window is known, prompts are fitted to it before the call — lowest-ranked documents first, then the oldest conversation turns, never the top-ranked document — and the answer says what was dropped, both as a one-line note and as a `미사용` badge on the sources that never reached the model (the source list still shows everything retrieval found). When it is unknown, nothing is trimmed: guessing a number and discarding evidence on it is worse than the overflow. A context overflow no longer trips the circuit breaker either — the error is deterministic, so blocking the only local provider just takes down the smaller requests that would have fit
- **Response modes (S / N / C)** — a per-message chat toggle picks the *character* of the answer, orthogonal to routing mode. **S** (brief) is one `## 요약` section under 1,000 characters — 1,500 on the Direct (no-retrieval) path, which has no document excerpts to lean on and has to spell the answer out itself — and skips verification entirely; **N** (standard, default) is the document-faithful five-section answer; **C** (creative) uses the retrieved documents as *material* to build new code/config, in a four-section format that separates what came from the documents from what the model filled in — and, being the only mode that writes anything the documents don't contain, it is the only one an operator can close off entirely (`CREATIVE_MODE_ENABLED`, hot-editable in `/settings`; the button disappears and the server downgrades any request that still asks for it). **Each mode owns a whole system prompt** rather than sharing one and overriding it from the user message — that override approach failed for S, where the shared prompt's list of five section headers beat the one-line instruction and the server truncated afterwards, leaving the screen and the stored answer disagreeing. **Only S names a character number**: a cap on short output sits before where the model stops on its own and binds, while a target on long output sits after it and does nothing (the removed `M`/`L` pair asked for ~5,000 and ~10,000 characters and produced 3,047 and 3,187 — that measurement is why `L` is gone). N and C say what to cover instead. `ResponseMode.maxTokens()` still derives a per-call budget (S 15%/2,000 floor, N and C 70%/5,000, never above `LLM_MAX_TOKENS`), but it is a runaway guard on **blocking** calls only — streaming answers have no token cap by design. `/settings` shows each mode's effective budget and which term won. **C is exclusive with Direct mode** (retrieval is its premise) — enforced on the client *and* independently on the server, and it is excluded from both the curated-knowledge axis and answer reuse (see below)
...
- **ReAct re-retrieval** — automatic retry up to 2 times when verification fails, and the two failures are treated differently: *insufficient* (didn't answer the question) re-searches, while *ungrounded* (went outside the documents) rewrites the answer against the evidence already in hand. A retry carries the evaluator's one-sentence rejection reason into the next prompt — without it the retry is a deterministic re-run of the same input at temperature 0. A re-search widens the candidate pool and the final cut, **and evicts** chunks the evaluator reported not using from the bottom of the ranking, so new evidence gets a slot instead of competing with dead weight
- **Critic verification** — one evaluation call judges sufficiency and groundedness together (the CRITIC node consumes that flag rather than making a second round-trip), against **the same documents the answer was written from** — the evidence window used to be the top 5 while the answer used all `SEARCH_TOP_K`, so an answer correctly citing a value found only in document #6-8 was judged as unfounded. Both prompts also see the same normalized form of each value. In response mode S the CRITIC stage is skipped and retries are driven only by the ANSWER sufficiency check. Mode C **replaces** the evaluator rather than skipping it: "is this grounded in the documents" is false by definition for a creative answer and would burn the whole retry budget, so it instead asks whether the identifiers the answer presented as document-sourced actually exist, and reports any invented ones as a warning that does not trigger a retry. **An unreadable verdict is "no verdict", not a pass** — if the evaluation call comes back empty or its JSON fails to parse, the turn is recorded with no verdict at all: no badge (neither `검증됨`/`생성` nor `미검증`) and no retry. The old fallback wrote `grounded=true` instead, which put a green *verified* badge on answers nothing had ever verified
...
- **PROGRESSIVE mode** — starts with COST_FIRST; when the verification call returns `sufficient=false` **and** the retry budget is already spent, re-runs Answer on a PREMIUM provider and marks the response with an upgrade badge. There is no quality score and no threshold property — `app.llm.progressive-threshold` was removed (2026-09-01) because nothing ever read it
...
- **Trusted client IP resolution** — `app.trust-forwarded-for` (default `false`) gates whether `X-Forwarded-For` is believed, for rate limiting, guest identity **and the audit log**. Off, a forged header can't refill an attacker's rate-limit bucket, impersonate another visitor, or write a false IP into the record of who did what; on (required behind Caddy/nginx), real client IPs are recovered instead of every visitor collapsing into the proxy address
- **Bounded log inputs** — a client-supplied `X-Trace-Id` is accepted only when it looks like an id (alphanumerics plus `_-`, 64 chars max) and replaced otherwise; it is printed on every log line, so an unchecked newline forged whole entries and a long value repeated on each one. The id follows the request into every thread it spawns (`MdcPropagation` — SSE worker, pre-run futures, retrieval axes, Vision calls, indexing workers and their nested executors) and into the async re-dispatch after an SSE error, so one `grep <traceId>` finds a whole turn or indexing job
- **Non-root container** — the image runs as UID 10001. Upgrading an existing Linux deployment needs a one-time `chown -R 10001:10001 ./data`; the entrypoint checks writability first and prints exactly that command rather than failing as a SQLite error
...
- **PPTX/PDF → Markdown conversion cleanup** — non-scanned PDF and PPTX convert to Markdown where a `[페이지: N]` marker (not a synthetic heading) is the per-page/slide section boundary; PPTX additionally drops image-less duplicate slides, agenda/table-of-contents slides (bullets that mostly match other slides' titles), title-only section-divider slides — numbered/keyword/short-noun-phrase titles like "PART 2"/"목차"/"결제 시스템", while sentence-like key-message titles are kept —, title-only "preview" slides whose title is repeated by the next slide, and a closing "끝"/"Thank you" slide (`app.pptx-remove-duplicate-slides`, `app.pptx-drop-divider-slides`, `app.pptx-drop-redundant-title-slides`, `app.pptx-drop-ending-slide`, all default on) — so content-free slides stay out of the search index
...
| `POST` | `/ui/chunk-reports` | Report a chunk as wrong/outdated from the source-text popup (§10.14, guest-open; admins get a "청크 수정" shortcut into `/admin` instead) |
| `GET/POST` | `/login` | Login page (auth mode, or no-auth management-only submode) |
| `GET/POST` | `/signup` | Sign-up page (auth mode only) |
| `GET/POST` | `/setup` | First-run admin setup — open in **every** auth mode, since this is the only path that creates a `ROLE_ADMIN` account (`/signup` only ever makes `ROLE_USER`) and the admin-gated surface is unreachable without one. Closes itself: redirects once an admin exists |
