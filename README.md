...
| `SEARCH_CURATED_QA_WEIGHT` | `1.0` | 0.5 ~ 5.0 | §10.10 — **👍-promoted** curated axis weight, applied flat like the keyword axis (not group-normalized with the vector axes). Both curated axes default to **`1.0`** — parity with the group-normalized vector group. Above that they surfaced loosely-related entries: these axes hold few candidates, so almost anything in them ranks high on its own axis, and a bonus on top of that amounts to "boost whatever exists here". Trustworthy and *relevant to this question* are different claims; relevance is what the RRF rank already measures. Raise one only with a reason (hot-editable). |
| `SEARCH_SUBMISSION_WEIGHT` | `1.0` | 0.5 ~ 5.0 | Weight of the **지식 제안** axis (approved user submissions). Both curated origins share one vector namespace and one search; they are split into two RRF axes by `MetaKey.CURATED_ORIGIN` so a hand-written, admin-reviewed entry can be weighted apart from a 👍 without raising both together — the split stays useful even though both now start at the same `1.0` |
