...
| `LOCAL_FAST_LLM_URL` | — | `http://localhost:8090/v1` | §6.21 task-tier offload — `providers[6]` (`local-fast`) endpoint, active by default in `application.properties`. Point it at a small (~500MB) model on a separate port/server; if nothing is listening there, `MICRO_TEXT` chores (keyword+context extraction, summary, title, query expansion) fail once, circuit-break, and fall back to the main LOCAL provider — see [OPERATOR_MANUAL.md §5.4 Example 6](documents/OPERATOR_MANUAL.md) |
| `LOCAL_FAST_LLM_KEY` | — | — | `providers[6]` API key. Optional for local endpoints, same as `LOCAL_LLM_KEY` |
| `LOCAL_FAST_LLM_MODEL` | — | `Qwen3.5-0.8B-Q4_K_M.gguf` | `providers[6]` model name |
