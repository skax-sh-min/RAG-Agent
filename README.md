...
- **Bounded log inputs** — a client-supplied `X-Trace-Id` is accepted only when it looks like an id (alphanumerics plus `_-`, 64 chars max) and replaced otherwise; it is printed on every log line, so an unchecked newline forged whole entries and a long value repeated on each one
